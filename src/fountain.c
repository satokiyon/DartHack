...
dowatersnakes(void)
{
    ...
    if (!(svm.mvitals[PM_WATER_MOCCASIN].mvflags & G_GONE)) {
        ...
        pline_The("泉はしばらく激しく泡立ったが、やがて静まった.");
    }
}
...
drinkfountain(void)
{
    ...
    if (fate < 10) {
        ...
        case 19: /* Self-knowledge */
            You_feel("自分のことがよくわかる...");
            display_nhwindow(WIN_MESSAGE, FALSE);
            enlightenment(MAGICENLIGHTENMENT, ENL_GAMEINPROGRESS);
            exercise(A_WIS, TRUE);
            pline_The("その感覚はやがて消えた.");
            break;
        case 20: /* Foul water */
            pline_The("水はひどくまずかった!  吐き気を催して吐いた.");
        ...
        case 26: /* See Monsters */
            if (monster_detect((struct obj *) 0, 0))
                pline_The("%sには何の味もしなかった.", hliquid("water"));
            exercise(A_WIS, TRUE);
            break;
...
dipfountain(struct obj *obj)
{
    ...
    if (obj->otyp == LONG_SWORD && u.ulevel >= 5
        ...
        static const char lady[] = "湖の乙女";

        if (u.ualign.type != A_LAWFUL) {
            /* Ha!  Trying to cheat her. */
            pline("%sから凍りつく霧が立ちのぼり"
                "、剣を包み込んだ.",
                  hliquid("water"));
            pline_The("泉は消えた!");
            curse(obj);
            ...
            exercise(A_WIS, TRUE);
            livelog_printf(LL_ARTIFACT, "%sは%sから授けられた",
    ...
    case 20: /* Uncurse the item */
        if (!is_hands && obj->cursed) {
            if (!Blind)
                pline_The("%sは一瞬光った.", hliquid("water"));
            uncurse(obj);
        } else {
            pline("何かを失ったような気分になった.");
        }
        break;
    case 21: /* Water Demon */
        dowaterdemon();
    ...
    case 29: /* You see coins */
        ...
        (void) mkgold((long) (rnd((dunlevs_in_dungeon(&u.uz) - dunlev(&u.uz)
                      ...
                      u.ux, u.uy);
        if (!Blind)
            pline("ずっと下の%sの中に硬貨がきらめいているのが見えた.",
                  hliquid("water"));
        exercise(A_WIS, TRUE);
        newsym(u.ux, u.uy);
        break;
...
wash_hands(void)
{
    ...
    You("%s%sを%sで洗った.", uarmg ? "手袋をした" : "", hands,
        hliquid("water"));
    if (Glib) {
        make_glib(0);
        Your("%sはもう滑らなくなった.", fingers_or_gloves(TRUE));
    }
    if (uarmg)
...
drinksink(void)
{
    ...
    switch (rn2(20)) {
    case 0:
        You("とても冷たい%sをひと口すすった.", hliquid("water"));
        break;
    case 1:
        You("とても温かい%sをひと口すすった.", hliquid("water"));
        break;
    case 2:
        You("焼けつくほど熱い%sをひと口すすった.", hliquid("water"));
        if (Fire_resistance) {
            pline("かなりおいしく感じた.");
    ...
    case 3:
        if (svm.mvitals[PM_SEWER_RAT].mvflags & G_GONE)
            pline_The("流し台はかなり汚れているようだ.");
        else {
            mtmp = makemon(&mons[PM_SEWER_RAT], u.ux, u.uy, MM_NOMSG);
            if (mtmp)
                pline("うわっ!  流し台に%sがいた!",
                      (Blind || !canspotmon(mtmp)) ? "うごめく何か"
                                                   : a_monnam(mtmp));
        }
        break;
    case 4:
        ...
        otmp->cursed = otmp->blessed = 0;
          pline("%s液体が蛇口から流れ出した.",
              Blind ? "妙な" : hcolor(OBJ_DESCR(objects[otmp->otyp])));
    ...
    case 5:
        if (!(levl[u.ux][u.uy].looted & S_LRING)) {
            ...
            pline("汚れた%sが排水口から逆流してきた.", hliquid("water"));
        break;
    case 6:
        breaksink(u.ux, u.uy);
        break;
    case 7:
        pline_The("%sがまるで意思を持つかのように動いた!", hliquid("water"));
        if ((svm.mvitals[PM_WATER_ELEMENTAL].mvflags & G_GONE)
            || !makemon(&mons[PM_WATER_ELEMENTAL], u.ux, u.uy, MM_NOMSG))
            pline("だが、すぐに静まった.");
        break;
    case 8:
        pline("うえっ、この%sはひどい味だった.", hliquid("water"));
        more_experienced(1, 0);
        newexplevel();
        break;
    case 9:
        pline("うげっ... 下水の味がした!  吐いてしまった.");
        morehungry(rn1(30 - ACURR(A_CON), 11));
        vomit();
        break;
    case 10:
        pline("この%sには有毒廃棄物が混じっていた!", hliquid("water"));
        if (!Unchanging) {
            You("異様な変身を遂げた!");
...
dipsink(struct obj *obj)
{
    ...
    if (!rn2(not_looted_yet ? 25 : 15)) {
        ...
        if (Glib && is_hands)
            Your("%sはまだ滑りやすかった.", fingers_or_gloves(TRUE));
        return;
    } else if (is_hands) {
        (void) wash_hands();
        return;
...
sink_backs_up(coordxy x, coordxy y)
{
    ...
    if (!Blind)
        Strcpy(buf, "泥まみれの汚水が排水口から噴き上がった");
    else if (!Deaf)
