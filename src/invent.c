    ...
    unsigned int ctrlflags) /* some control to fine-tune the behavior */
{
 ...
 redo_menu:
        ...
        if (!otmp) {
            ...
            You("そんなに持っていない!  持っているのは%ldだけだった.", otmp->quan);
            if (gi.in_doagain)
                return (struct obj *) 0;
            continue;
        }
...
doperminv(void)
{
    ...
    if ((windowprocs.wincap & WC_PERM_INVENT) == 0) {
        /* [TODO? perhaps omit "by <interface>" if all the window ports
          ...
          pline("持続型インベントリ表示は '%s' では対応していない.",
              windowprocs.name);
...
#define OVRFLW_INDX (GOLD_OFFSET + invlet_basic) /* past gold+2*26 letters */
 ...
 noadjust:
        ...
        if (trycnt == 5)
            goto noadjust;
        pline("インベントリのスロット文字を選んでほしい."); /* else try again */
    }
