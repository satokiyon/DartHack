    ...
    if (negated) {
        ...
        if (verbosely) {
            if (mdef == &gy.youmonst)
                ...
                pline_mon(mdef, "%sは危害を免れた.", Monnam(mdef));
        }
        return TRUE;
    }
    return FALSE;
    ...
    struct obj *wep)    /* uwep for do_attack(), null for kick_monster() */
{
    ...
    if (!canspotmon(mtmp)
        ...
        pline("待って! そこには見えない%sがいる!", something);
        map_invisible(gb.bhitpos.x, gb.bhitpos.y);
    ...
    if (mtmp->mundetected && !canseemon(mtmp)
        ...
        if (!tp_sensemon(mtmp) && !Detect_monsters) {
            ...
            if (!Blind && Hallucination)
                ...
                pline("待って! そこには隠れたモンスターがいる!");
            else if ((obj = svl.level.objects[mtmp->mx][mtmp->my]) != 0)
                pline("待って! %sの下に%sが隠れている!",
                      doname(obj),
                      notseen ? something : (const char *) lmonbuf);
            return TRUE;
        }
    }
    ...
    if (flags.confirm && mtmp->mpeaceful
        ...
        if (canspotmon(mtmp)) {
            ...
            Sprintf(qbuf, "%sを本当に攻撃しますか?", mon_nam(mtmp));
    ...
    if (is_safemon(mtmp) && !svc.context.forcefight) {
        if (!u_wield_art(ART_STORMBRINGER)) {
            ...
            if (inshop || foo) {
                ...
                buf[0] = highc(buf[0]);
                You("立ち止まった. %sが行く手を塞いでいた!", buf);
                end_running(TRUE);
                return TRUE;
            } else if (mtmp->mfrozen || helpless(mtmp)
                       || (mtmp->data->mmove == 0 && rn2(6))) {
                pline("%sは動く気配がない.", Monnam(mtmp));
                end_running(TRUE);
                return TRUE;
            } else
                return FALSE;
        ...
        hmd->hittxt = TRUE;
               ...
               && ((monwep = MON_WEP(mon)) != 0
                   ...
                   && !obj_resists(monwep,
                                   ...
                                   100))) {
        static const char from_your_blow[] = " あなたの強烈な一撃だ!";
        char buf[BUFSZ];
        /*
        ...
        if (canseemon(mon))
            ...
            /* construct "Its weapon shatters"; not an exact replacement
               ...
               monster detection will name mon but not its weapon */
                Sprintf(buf, "%sの武器が砕け散る", Monnam(mon));
...
#undef useup_eggs
    ...
    case BLINDING_VENOM:
        ...
        if (can_blnd(&gy.youmonst, mon,
            ...
            pline(obj->otyp == CREAM_PIE ? "べちゃっ!" : "ばしゃっ!");
            setmangry(mon, TRUE);
        }
        {
            boolean more_than_1 = (obj->quan > 1L);
    ...
    case ACID_VENOM: /* thrown (or spit) */
        if (resists_acid(mon)) {
            Your("毒液は%sに無害に当たった.", l_monnam(mon));
            hmd->dmg = 0;
        } else {
            Your("毒液が%sを焼いた!", l_monnam(mon));
            hmd->dmg = dmgval(obj, mon);
        }
        {
            boolean more_than_1 = (obj->quan > 1L);
    ...
    struct obj *obj) /* lance; obj is not NULL */
{
    ...
    You("%sへ突撃した%s", l_monnam(mon), canseemon(mon) ? exclam(hmd->dmg) : ".");
    ...
    struct obj *obj UNUSED)
{
    ...
    if (rnd(100) < P_SKILL(P_BARE_HANDED_COMBAT) && !bigmonst(hmd->mdat)
        ...
        if (canspotmon(mon))
            pline("%sはあなたの強烈な一撃でよろめいた!", Monnam(mon));
        if (mhurtle_to_doom(mon, hmd->dmg, &hmd->mdat))
            hmd->already_killed = TRUE;
        hmd->hittxt = TRUE;
    }
    ...
    int dieroll)
{
    ...
    if (hmd.needpoismsg)
        pline_The("毒は%sに効いていないようだ.", l_monnam(mon));
    if (hmd.poiskilled) {
        pline_The("毒は致命的だった...");
        if (!hmd.already_killed)
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(3) && !(resists_drli(mdef) || defended(mdef, AD_DRLI))
            ...
            if (DEADMONSTER(mdef) || !mdef->m_lev) {
                    pline("%sは%s!", Monnam(mdef),
                        nonliving(mdef->data) ? "活動を停止した" : "死んだ");
                xkilled(mdef, XKILL_NOMSG);
            } else
                mdef->m_lev--;
        ...
        if (is_death
            ...
            if (gv.vis && canspotmon(mdef))
                pline_mon(mdef, "%sは弱っていく!", Monnam(mdef));
            if (mdef->mhpmax - mhm->damage > (int) mdef->m_lev) {
                mdef->mhpmax -= mhm->damage;
            } else {
                /* limit floor of mhpmax reduction to current m_lev + 1;
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (resists_fire(mdef) || defended(mdef, AD_FIRE)) {
            if (!Blind)
                pline_The("炎は%sを熱していない!", l_monnam(mdef));
        ...
        if (resists_fire(mdef) || defended(mdef, AD_FIRE)) {
            if (gv.vis && canseemon(mdef))
                pline_The("炎は%sを焼いていないようだ!", l_monnam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_FIRE, mhm->damage);
            mhm->damage = 0;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!Blind)
            pline("%sは霜に包まれた!", Monnam(mdef));
        if (resists_cold(mdef) || defended(mdef, AD_COLD)) {
            shieldeff(mdef->mx, mdef->my);
            if (!Blind)
                pline_The("霜気は%sを凍えさせない!", l_monnam(mdef));
        ...
        if (gv.vis && canseemon(mdef))
            pline_mon(mdef, "%sは霜に包まれた!", Monnam(mdef));
        if (resists_cold(mdef) || defended(mdef, AD_COLD)) {
            if (gv.vis && canseemon(mdef))
                pline_The("霜気は%sを凍えさせていないようだ!", l_monnam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_COLD, mhm->damage);
            mhm->damage = 0;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!Blind)
            pline("%sは電撃を受けた!", Monnam(mdef));
        if (resists_elec(mdef) || defended(mdef, AD_ELEC)) {
            if (!Blind)
                pline_The("電撃は%sに効かない!", l_monnam(mdef));
        ...
        if (gv.vis && canseemon(mdef))
            pline_mon(mdef, "%sは電撃を受けた!", Monnam(mdef));
        if (resists_elec(mdef) || defended(mdef, AD_ELEC)) {
            if (gv.vis && canseemon(mdef))
                pline_The("電撃は%sに効かない!", l_monnam(mdef));
            shieldeff(mdef->mx, mdef->my);
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (magr->mcan || mhm->damage >= mdef->mhp || tele_restrict(mdef)) {
            ...
            if (gv.vis)
                pline_mon(mdef, "%sには効かなかった.", Monnam(mdef));
        } else {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated && !rn2(8)) {
            ...
            if (resists_poison(mdef)) {
                pline_The("毒は%sに効いていないようだ.", l_monnam(mdef));
            } else {
                if (!rn2(10)) {
                    Your("毒は致命的だった...");
                    mhm->damage = mdef->mhp;
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (gn.notonhead || !has_head(pd)) {
            pline("%sには効いていないようだ.", Monnam(mdef));
        ...
        if (!sticks(pd)) {
            ...
            if (!u.ustuck && !tailmiss && !rn2(10)) {
                ...
                if (is_pool(u.ux, u.uy) && !cant_drown(pd)) {
                    You("%sを溺れさせた...", l_monnam(mdef));
                    mhm->damage = mdef->mhp;
                } else if (mattk->aatyp == AT_HUGS)
                    pline("%sは締めつけられている.", Monnam(mdef));
            } else {
                mhm->damage = 0;
                if (flags.verbose) {
                    if (coil && !tailmiss)
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(3) && mhm->damage < mdef->mhp
            && !mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            if (!Blind)
                pline("%sはあなたの攻撃で凍りついた!", Monnam(mdef));
        ...
        if (mdef->mcanmove && !rn2(3)
            ...
            if (gv.vis && canspotmon(mdef)) {
                ...
                pline("%sは%sに凍らされた.", buf, mon_nam(magr));
            }
            paralyze_monst(mdef, rnd(10));
        }
    }
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated) {
            ...
            if (obj && drain_item(obj, FALSE)) {
                pline("%sの効果が弱まった.", Yobjnam2(obj, "seem"));
            }
        }
    } else {
        /* mhitm */
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated && mdef->mspeed != MSLOW) {
            ...
            if (mdef->mspeed != oldspeed && gv.vis && canspotmon(mdef))
                pline_mon(mdef, "%sは動きが鈍くなった.", Monnam(mdef));
        }
    }
}
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && !mdef->mconf && !magr->mspec_used) {
            if (gv.vis && canseemon(mdef))
                pline_mon(mdef, "%sは混乱したようだ.", Monnam(mdef));
            mdef->mconf = 1;
            mdef->mstrategy &= ~STRAT_WAITFORU;
        }
    }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        /* mhitu */
        pline_mon(magr, "%sが手を伸ばし、あなたの体はしぼんでいく!",
                  Monnam(magr));
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        /* mhitu */
        pline_mon(magr, "%sが手を伸ばし、あなたは熱と寒気に襲われた!",
                  Monnam(magr));
        (void) diseasemu(pa);
        /* plus the normal damage */
    } else {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        /* mhitu */
        pline_mon(magr, "%sが死の手を伸ばした!", Monnam(magr));
        if (is_undead(pd)) {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && haseyes(pd) && mdef->mcansee) {
            if (gv.vis && canseemon(mdef))
                pline_mon(mdef, "%sは%s混乱したようだ.", Monnam(mdef),
                      mdef->mconf ? "さらに" : "");
            mdef->mconf = 1;
            mdef->mstrategy &= ~STRAT_WAITFORU;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (!resists_ston(mdef)) {
        if (gv.vis && canseemon(mdef))
            pline_mon(mdef, "%sは石に変わった!", Monnam(mdef));
        monstone(mdef);
 post_stone:
        if (!DEADMONSTER(mdef)) {
            mhm->hitflags = M_ATTK_MISS;
        ...
        if (mattk->aatyp == AT_HUGS && !sticks(pd)) {
            if (!u.ustuck && rn2(2)) {
                if (u_slip_free(magr, mattk)) {
                    ...
                    set_ustuck(magr);
                    pline_mon(magr, "%sがあなたをつかんだ!", Monnam(magr));
                    mhm->hitflags |= M_ATTK_HIT;
                }
            } else if (u.ustuck == magr) {
                exercise(A_STR, FALSE);
                You("%sれていた.",
                    (pa == &mons[PM_ROPE_GOLEM]) ? "締め上げら" : "押し潰さ");
            }
        ...
        if (shade_miss(magr, mdef, mwep, FALSE, vis)) {
            ...
            if (mwep->oartifact) {
                ...
                if (!artifact_hit(magr, mdef, mwep, &mhm->damage,
                                  mhm->dieroll)) {
                    if (gv.vis)
                        pline_mon(magr, "%sが%sを攻撃した.", Monnam(magr),
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(3)) {
            if (magr->mcan) {
                if (!Deaf)
                        You_hear("%sのせき込みが聞こえる!", l_monnam(magr));
            } else {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!(uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))
            ...
            pline_mon(magr, "%sはあなたをたたいた! (気を悪くしないでね。)",
                      Monnam(magr));
            if (Upolyd) {
                u.mh += rnd(7);
                if (!rn2(7)) {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (magr->mcan)
            return;
        if (canseemon(mdef))
            pline_mon(mdef, "%sは一瞬よろめいた.", Monnam(mdef));
        mdef->mstun = 1;
        mhitm_ad_phys(magr, mattk, mdef, mhm);
        if (mhm->done)
            return;
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if ((u.usteed || Levitation || Flying) && !is_flyer(magr->data)) {
            pline("%sはあなたの%s%sを狙ったが届かなかった!", Monst_name,
                  sidestr, leg);
            mhm->damage = 0;
        } else if (magr->mcan) {
            pline_mon(magr, "%sはあなたの%s%sに鼻先をこすりつけた!", Monnam(magr),
                  sidestr, leg);
            mhm->damage = 0;
        } else {
            if (uarmf) {
                if (rn2(2) && (uarmf->otyp == LOW_BOOTS
                    ...
                    pline("%sはあなたの露出した%s%sを突いた!",
                          Monst_name, sidestr, leg);
                } else if (!rn2(5)) {
                    pline("%sはあなたの%sブーツを突き抜けた!", Monst_name,
                          sidestr);
                } else {
                    pline("%sはあなたの%sブーツをひっかいた!", Monst_name,
                          sidestr);
                    mhm->damage = 0;
                    return;
                }
            } else
                pline("%sはあなたの%s%sを突いた!", Monst_name, sidestr, leg);
    ...
    if (magr == &gy.youmonst) {
        ...
        default:
            ...
            if (is_animal(magr->data) && *buf) {
                if (canseemon(magr))
                    pline_mon(magr, "%sは%sを抱えて逃げようとした.",
                              Monnam(magr), buf);
            }
...
explum(struct monst *mdef, struct attack *mattk)
{
    ...
    case AD_BLND:
        if (mdef && !resists_blnd(mdef)) {
            pline("%sはあなたの閃光で目がくらんだ!", Monnam(mdef));
    ...
    case AD_HALU:
        if (mdef && haseyes(mdef->data) && mdef->mcansee) {
            pline("%sはあなたの閃光で混乱した!", Monnam(mdef));
            mdef->mconf = 1;
        }
        break;
    case AD_COLD:
        ...
        if (fatal_gulp && !is_rider(pd)) { /* petrification */
            ...
            case AD_DGST:
                ...
                if (!DEADMONSTER(mdef)) { /* monster lifesaved */
                    ...
                    Sprintf(msgbuf, "%sを完全に消化した.", mon_nam(mdef));
                    if (tmp != 0) {
            ...
            case AD_PHYS:
                if (gy.youmonst.data == &mons[PM_FOG_CLOUD]) {
                    pline("%sはあなたの湿気をまとった.", Monnam(mdef));
                    ...
                    pline("%sは%s!", Monnam(mdef),
                                                    enfolds(gy.youmonst.data) ? "押し潰されている"
                                                        : "体内の破片で打ち据えられている");
                }
                break;
            case AD_ACID:
                pline("%sはあなたの粘液を浴びた!", Monnam(mdef));
                if (resists_acid(mdef)) {
                    pline("%sには効いていないようだ.", mon_nam(mdef));
                    dam = 0;
                }
                break;
            case AD_BLND:
                if (can_blnd(&gy.youmonst, mdef, mattk->aatyp,
                             (struct obj *) 0)) {
                    if (mdef->mcansee)
                        pline("%sは中で目が利かないようだ!", Monnam(mdef));
                    mdef->mcansee = 0;
                    dam += mdef->mblinded;
                    if (dam > 127)
                        dam = 127;
            ...
            case AD_COLD:
                if (rn2(2)) {
                    if (resists_cold(mdef)) {
                        pline("%sは少し寒そうだ.", Monnam(mdef));
                        dam = 0;
                    } else
                        pline("%sは凍え死にそうだ!", Monnam(mdef));
                    golemeffects(mdef, (int) mattk->adtyp, dam);
                } else
                    dam = 0;
                break;
            case AD_FIRE:
                if (rn2(2)) {
                    if (resists_fire(mdef)) {
                        pline("%sは少し熱そうだ.", Monnam(mdef));
                        dam = 0;
                    } else
                        pline("%sはこんがり焼けそうだ!", Monnam(mdef));
                    golemeffects(mdef, (int) mattk->adtyp, dam);
                } else
                    dam = 0;
                break;
    ...
    boolean wouldhavehit)
{
    ...
    if (could_seduce(&gy.youmonst, mdef, mattk))
        You("%sに友好的なふりをした.", l_monnam(mdef));
    else if (canspotmon(mdef) && flags.verbose)
        You("%sへの攻撃を外した.", l_monnam(mdef));
    else
        You("外した.");
    if (!helpless(mdef))
        wakeup(mdef, TRUE);
    ...
    if (m_is_steadfast(mdef)) {
        if (u_def || (u.usteed && mdef == u.usteed)) {
            ...
            You("%sびくともしなかった.", mdefbuf);
                        ...
                        pline("%sはびくともしない.", Monnam(mdef));
        }
        return FALSE;
    }

    /* subtly vary the message text if monster won't actually move */
        knockedhow = dismount ? "鞍から"
                                 : will_hurtle(mdef, defx + dx, defy + dy) ? "後方へ"
                                     : "後ろへ";

    /* give the message */
    if (u_def || canseemon(mdef)) {
        Strcpy(magrbuf, u_agr ? "あなた" : Monnam(magr));
        Strcpy(mdefbuf, (u_def || was_u) ? "あなた" : y_monnam(mdef));
        if (was_u)
            Snprintf(eos(mdefbuf), sizeof mdefbuf - strlen(mdefbuf),
                     "と%s", y_monnam(u.usteed));
        /*
 ...
 use_weapon:
        ...
        case AT_HUGS: {
            ...
            if (mon->data == &mons[PM_SHADE]) {
                const char *verb = byhand ? "つかんだ" : "抱きしめた";
                ...
                if (specialdmg) {
                    You("%sを%s%s", mon_nam(mon), verb, exclam(specialdmg));
                    if (silverhit && flags.verbose)
                        silver_sears(&gy.youmonst, mon, silverhit);
                    sum[i] = damageum(mon, mattk, specialdmg);
                } else {
                    Your("%sは無害に%sをすり抜けた.",
                         byhand ? "つかみ" : "抱擁", mon_nam(mon));
                }
                break;
            }
        ...
        case AT_ENGL:
            ...
            if ((dhit = (tmp > rnd(20 + i)))) {
                ...
                if (mon->data == &mons[PM_SHADE]) {
                    ...
                    Your("%sを包み込もうとしたが無害だった.", l_monnam(mon));
    ...
    case AD_MAGM:
        ...
        if (Antimagic) {
            ...
            monstseesu(M_SEEN_MAGR);
                pline("魔法の矢の雨があなたをかすめていった!");
        } else {
            You("どこからともなく現れたマジックミサイルに当たった!");
            mdamageu(mon, tmp);
    ...
    if (Blind) {
        ...
        if (glyph_is_cmap(glyph)) {
            ...
            if (M_AP_TYPE(mtmp) == M_AP_FURNITURE
                ...
                Snprintf(fmtbuf, sizeof fmtbuf, "その%sは実は%%sだ!",
                         jp_cmap_explanation(sym, x, y));
    ...
    struct obj *otmp) /* source of flash */
{
    ...
    if (mtmp->msleeping && haseyes(mtmp->data)) {
        mtmp->msleeping = 0;
        if (useeit) {
            pline_The("閃光で%sは目を覚ました.", l_monnam(mtmp));
            res = 1;
        }
        ...
        if (!resists_blnd(mtmp)) {
            ...
            if (flags.verbose) {
                if (lev->lit)
                    pline("閃光が%sを照らした.", mon_nam(mtmp));
                else
                    pline("%sが照らし出された.", Monnam(mtmp));
...
light_hits_gremlin(struct monst *mon, int dmg)
{
    if (!Deaf && mdistu(mon) <= 90) {
        ...
        pline_mon(mon, "%sは光にひるんだ!", Monnam(mon));
    }
    mon->mhp -= dmg;
