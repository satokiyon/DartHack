        ...
        if (canseemon(mdef) && !sensemon(mdef)) {
            if (Unaware) {
                ...
                if (iflags.last_msg == PLNMSG_HIDE_UNDER
                    ...
                    pline_mon(mdef, "%sは隠れ場所から現れた.", l_monnam(mdef));
                else if (mdef->m_id == gl.last_hider)
                    You("%sに気づいた。", l_monnam(mdef));
                else
        ...
        if ((gv.vis && canspotmon(mdef)) /* mon-vs-mon */
            ...
            boolean tailmiss = gn.notonhead;
               ...
               names to overcome that [note: comment predates 'tailmiss'] */
                Strcpy(magrnam, (magr == &gy.youmonst) ? "あなた"
                                                                    : Monnam(magr));
            if (!tailmiss) {
...
gazemm(struct monst *magr, struct monst *mdef, struct attack *mattk)
{
    ...
    if (magr->data == &mons[PM_MEDUSA] && mon_reflects(mdef, (char *) 0)) {
        if (canseemon(mdef))
            (void) mon_reflects(mdef, "視線は%s%sで反射された.");
        if (mdef->mcansee) {
...
explmm(struct monst *magr, struct monst *mdef, struct attack *mattk)
{
    ...
    if (cansee(magr->mx, magr->my))
        pline_mon(magr, "%sは爆発した!", l_monnam(magr));
    else
        noises(magr, mattk);
...
slept_monst(struct monst *mon)
{
    if (helpless(mon) && mon == u.ustuck
        ...
        pline_mon(mon, "%sは締めつけを緩めた.", Monnam(mon));
        unstuck(mon);
    }
}
    ...
    if (rn2(3))
        ...
        case AD_COLD:
            if (resists_cold(magr)) {
                if (canseemon(magr)) {
                    pline_mon(magr, "%sは少し冷えた.", l_monnam(magr));
            ...
            if (canseemon(magr))
                pline_mon(magr, "%sは突然とても冷たくなった!", l_monnam(magr));
        ...
        case AD_FIRE:
            if (resists_fire(magr)) {
                if (canseemon(magr)) {
                    pline_mon(magr, "%sは少し温まった.", l_monnam(magr));
            ...
            if (canseemon(magr))
                pline_mon(magr, "%sは突然とても熱くなった!", l_monnam(magr));
            break;
        case AD_ELEC:
            if (resists_elec(magr)) {
                if (canseemon(magr)) {
                    pline_mon(magr, "%sは少ししびれた.", l_monnam(magr));
...
xdrainenergym(struct monst *mon, boolean givemsg)
{
    if (mon->mspec_used < 20 /* limit draining */
        ...
        if (givemsg)
            pline_mon(mon, "%sはけだるそうになった.", l_monnam(mon));
    }
}
