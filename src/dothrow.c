    ...
    boolean verbosely) /* usually True; False if caller has given drop mesg */
{
    ...
    if (IS_ALTAR(levl[u.ux][u.uy].typ)) {
        ...
        pline("%sが%sに当たった.", Doname2(obj), surf);
    }
...
mhurtle_step(genericptr_t arg, coordxy x, coordxy y)
{
    ...
    if ((mtmp = m_at(x, y)) != 0 && mtmp != mon) {
        ...
        pline("%sがあなたにぶつかった.", Some_Monnam(mon));
        stop_occupation();
...
toss_up(struct obj *obj, boolean hitsroof)
{
    ...
    if (!has_ceiling(&u.uz)) {
        action = "飛び上がって"; /* into "the sky" or "the water above" */
    } else if (hitsroof) {
        if (breaktest(obj)) {
            pline("%sが%sに当たった.", Doname2(obj), ceiling(u.ux, u.uy));
            breakmsg(obj, !Blind);
    ...
    if (obj->oclass == POTION_CLASS) {
        ...
        case EGG:
            if (petrifier && !Stone_resistance
                ...
                if (uarmh)
                    Your("%sでは身を守れなかった.", helm_simple_name(uarmh));
                goto petrify;
            }
            FALLTHROUGH;
            /*FALLTHRU*/
...
throw_gold(struct obj *obj)
{
    ...
    if (flooreffects(obj, gb.bhitpos.x, gb.bhitpos.y, "fall"))
        return ECMD_TIME;
    if (u.dz > 0)
        pline_The("金貨は%sに当たった.", surface(gb.bhitpos.x, gb.bhitpos.y));
    place_object(obj, gb.bhitpos.x, gb.bhitpos.y);
    if (*u.ushops)
