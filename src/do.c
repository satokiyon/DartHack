...
dosinkring(struct obj *obj)
{
 ...
 giveback:
    ...
    if (!rn2(20) && !nosink) {
        pline_The("シンクが水を吐い出して%sを残した.", doname(obj));
        obj->in_use = FALSE;
        dropx(obj);
    } else if (!rn2(5)) {
        freeinv(obj);
...
canletgo(struct obj *obj, const char *word)
{
    if (obj->owornmask & (W_ARMOR | W_ACCESSORY)) {
        if (*word)
            Norep("着用中の%sを%sんでいる不接で、%sを%sことができない.", "something", "wearing", "something", word);
        return FALSE;
    }
    ...
    if (obj->otyp == LEASH && obj->leashmon != 0) {
        if (*word)
            pline_The("紅い笼をあなたの%sに紅い笼が巻きついている.", body_part(HAND));
        return FALSE;
    }
    if (obj->owornmask & W_SADDLE) {
        if (*word)
            You("あなたが乗っている%sを%sことができない.", "something", word);
        return FALSE;
    }
    return TRUE;
}
...
u_stuck_cannot_go(const char *updn)
{
    if (u.ustuck) {
        if (u.uswallow || !sticks(gy.youmonst.data)) {
            You("%s%s、%sへ業が追う、ぜひ、そこへ訓緱してください.",
                !u.uswallow ? "何かが"
                : digests(u.ustuck->data) ? "食い込まれて"
                : "engulfed", updn);
            return TRUE;
        } else {
