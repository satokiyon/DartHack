    ...
    const char *name) /* if null, then format `*objp' */
{
    ...
    if (u.uac + tlev <= (dieroll = rnd(20))) {
        ...
        if (Blind || !flags.verbose) {
            ...
            pline("%sの攻撃はあなたに当たらなかった.", upstart(onmbuf));
        } else
            You("%sに当たりそうになった.", onm);
        return 0;
    } else {
        if (Blind || !flags.verbose)
            ...
            You("%sの攻撃を受けた%s", onm, exclam(dam));

        if (is_acid && Acid_resistance) {
            pline("それはあなたに効かないようだ.");
...
monshoot(struct monst *mtmp, struct obj *otmp, struct obj *mwep)
{
    ...
    if (canseemon(mtmp)) {
        ...
        set_msg_xy(mtmp->mx, mtmp->my);
          if (mtarg)
            pline("%sは%sを%sへ%s!", Monnam(mtmp), onm, trgbuf,
                gm.m_shot.s ? "撃った" : "投げた");
          else
    ...
    if (tmp < rnd(20)) {
        ...
        if (!DEADMONSTER(mtmp)
            ...
            if (vis && mtmp->mcansee)
                ...
                pline("%sは%sで目がくらんだ.", Monnam(mtmp),
                      ((otmp->oclass == VENOM_CLASS) ? "毒液"
...
thrwmu(struct monst *mtmp)
{
    ...
    if (is_pole(otmp)) {
        ...
        if (canseemon(mtmp)) {
            onm = xname(otmp);
            pline_mon(mtmp, "%sは%sで攻撃した.", Monnam(mtmp),
