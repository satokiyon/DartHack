...
static const struct RoleName jp_role_ranks[NUM_ROLES][9] = {
    ...
    {
        { "騎士見習い", 0 }, { "従士", 0 }, { "准騎士", 0 },
        { "下級騎士", 0 }, { "騎士", "女騎士" }, { "旗手騎士", 0 },
        { "高位騎士", "高位女騎士" }, { "騎士長", "女騎士長" }, { "聖騎士", 0 }
    },
    {
        { "入門希望者", 0 }, { "初心修行者", 0 }, { "修行僧", 0 },
        { "土の使い手", 0 }, { "水の使い手", 0 }, { "金の使い手", 0 },
        { "木の達人", 0 }, { "火の達人", 0 }, { "免許皆伝", 0 }
    },
    {
        { "修道者", "修道女" }, { "侍者", 0 }, { "侍祭", 0 },
        { "僧侶", "尼僧" }, { "助任司祭", 0 }, { "聖者", "聖女" },
    ...
    },
    {
        { "手品師", 0 }, { "奇術師", 0 }, { "占い師", 0 },
        { "霊感師", 0 }, { "召喚師", 0 }, { "妖術師", 0 },
        { "魔術師", 0 }, { "魔法使い", 0 }, { "大魔法使い", 0 }
    }
};

const char *
jp_rank_of_for_display(int lev, short monnum, boolean female)
{
    const struct Role *role;
    int i, roleidx;

    for (roleidx = 0, role = roles; role->name.m; roleidx++, role++)
        if (monnum == role->mnum)
            break;
    if (!role->name.m) {
        roleidx = Role_switch;
    ...
    for (i = xlev_to_rank((int) lev); i >= 0; i--) {
        if (female && jp_role_ranks[roleidx][i].f)
            return jp_role_ranks[roleidx][i].f;
        if (jp_role_ranks[roleidx][i].m)
            return jp_role_ranks[roleidx][i].m;
    }

    return jp_role_name_for_display(roleidx, female ? 1 : 0);
}
