...
#ifndef NH_C3_KATAKANA
#define NH_C3_KATAKANA   0x0010U
#endif
#ifndef NH_C3_HIRAGANA
#define NH_C3_HIRAGANA   0x0020U
#endif
#ifndef NH_C3_IDEOGRAPH
#define NH_C3_IDEOGRAPH  0x0100U
#endif

static int
nh_win32_cell_expected_width(const cell_t *cell)
{
    ...
    if (GetStringTypeW(CT_CTYPE3, &wch, 1, &chartype)) {
        if ((chartype & C3_NONSPACING) != 0U)
            return 0;
        if ((chartype & (C3_FULLWIDTH | NH_C3_KATAKANA
                         | NH_C3_HIRAGANA | NH_C3_IDEOGRAPH)) != 0U)
            return 2;
    }

    /* Console renders these as full-width in this environment. */
    switch (wch) {
    case 0x3005: /* 々 */
    case 0x300E: /* 『 */
    case 0x300F: /* 』 */
...
static boolean
nh_win32_is_wide_trailing_cell(const cell_t *back, COORD pos)
{
    const cell_t *prev;

    if (!back || pos.X <= 0)
        return FALSE;
    if (back->wcharacter != CONSOLE_CLEAR_CHARACTER)
        return FALSE;

    prev = back - 1;
    if (!prev->wcharacter || prev->wcharacter == CONSOLE_CLEAR_CHARACTER)
        return FALSE;
