...
Cコアの `include/seffects.h` に定義されている 235 種の効果音 ID の全一覧です（戦闘アクション効果音 33種を含む）。
...
| 68 | `se_door_open.ogg` | `se_door_open` | ドアがギィッと開く音（通常の開扉） | monmove.c |
| 69 | `se_door_unlock_and_open.ogg` | `se_door_unlock_and_open` | 鍵がカチャリと解錠されてドアが開く音 | monmove.c |
| 70 | `se_drain_noises.ogg` | `se_drain_noises` | 排水口がゴボゴボと鳴る音（排水溝近く） | do.c |
| 71 | `se_dry_throat_rattle.ogg` | `se_dry_throat_rattle` | 乾燥した喉がガラガラ鳴る音（モンスターの息） | mthrowu.c |
| 72 | `se_egg_cracking.ogg` | `se_egg_cracking` | 卵の殻がパキッと割れる音（蹴った時） | dokick.c |
| 73 | `se_egg_splatting.ogg` | `se_egg_splatting` | 卵がグシャッと踏み潰れる音 | dokick.c |
| 74 | `se_elephant_trumpet.ogg` | `se_elephant_trumpet` | 象のパオーンという高らかな鳴き声 | sounds.c |
| 75 | `se_equine_neigh.ogg` | `se_equine_neigh` | 馬のヒヒーンという高いいななき | sounds.c |
| 76 | `se_equine_whicker.ogg` | `se_equine_whicker` | 馬の低く柔らかいいななき | sounds.c |
| 77 | `se_equine_whinny.ogg` | `se_equine_whinny` | 馬の短いいななき声 | sounds.c |
| 78 | `se_explosion.ogg` | `se_explosion` | 大爆発音（錠前爆発・爆破罠） | lock.c |
| 79 | `se_faint_chime.ogg` | `se_faint_chime` | かすかなチリンというチャイムの音（魔法感知） | spell.c |
| 80 | `se_faint_sloshing.ogg` | `se_faint_sloshing` | かすかな水が揺れる音（アイテム生成時） | mkobj.c |
| 81 | `se_faint_splashing.ogg` | `se_faint_splashing` | かすかな水しぶき音（ホイールへの接触） | apply.c |
| 82 | `se_feline_meow.ogg` | `se_feline_meow` | 猫のニャーという鳴き声 | sounds.c |
| 83 | `se_feline_mew.ogg` | `se_feline_mew` | 子猫のミューという細い鳴き声 | sounds.c |
| 84 | `se_feline_purr.ogg` | `se_feline_purr` | 猫のゴロゴロと喉を鳴らす音 | sounds.c |
| 85 | `se_feline_yelp.ogg` | `se_feline_yelp` | 猫のギャッという悲鳴 | 予備（直接呼び出しなし） |
| 86 | `se_feline_yip.ogg` | `se_feline_yip` | 猫の甲高いキーという鳴き声 | 予備（直接呼び出しなし） |
| 87 | `se_feline_yowl.ogg` | `se_feline_yowl` | 猫のウウウという低い唸り声 | sounds.c |
| 88 | `se_furious_bubbling.ogg` | `se_furious_bubbling` | 液体が激しくボコボコと泡立つ音（噴水の呪い） | fountain.c |
| 89 | `se_gear_turn.ogg` | `se_gear_turn` | 歯車がカチリと一段回る音（機構の動作） | music.c |
| 90 | `se_gears_turning_chains_rattling.ogg` | `se_gears_turning_chains_rattling` | 歯車が回り鎖が引き上げられる音（跳ね橋の引き上げ） | dbridge.c |
| 91 | `se_glass_crashing.ogg` | `se_glass_crashing` | ガラスが激しくバリーンと割れる音 | dokick.c |
| 92 | `se_glass_shattering.ogg` | `se_glass_shattering` | ガラス瓶が粉々にパリーンと砕け散る音 | dokick.c |
| 93 | `se_groan.ogg` | `se_groan` | ウゥーといううめき声（モンスターや地形） | sounds.c |
| 94 | `se_groans_and_moans.ogg` | `se_groans_and_moans` | 呻きとうめき声（アンデッドの気配・死体部屋） | do.c |
| 95 | `se_growl.ogg` | `se_growl` | ウーンという低い唸り声 | 予備（直接呼び出しなし） |
| 96 | `se_grunt.ogg` | `se_grunt` | フンと鼻を鳴らす短い声 | sounds.c |
| 97 | `se_guards_footsteps.ogg` | `se_guards_footsteps` | 警備員の重い足音が近づく音（金庫室侵入） | sounds.c |
| 98 | `se_gurgle.ogg` | `se_gurgle` | ゴボゴボと湧き立つ液体の音（モンスターの声） | sounds.c |
| 99 | `se_gushing_sound.ogg` | `se_gushing_sound` | ドバーッと液体や水が噴き出す音（配管破壊） | dokick.c |
| 100 | `se_heart_beat.ogg` | `se_heart_beat` | トックン、トックンという心臓の鼓動音（聴診器使用） | apply.c |
| 101 | `se_hiss.ogg` | `se_hiss` | シュッというヘビや蒸気の威嚇音 | sounds.c |
| 102 | `se_hollow_sound.ogg` | `se_hollow_sound` | コンコンと中が空洞に響く音（壁や箱の確認） | apply.c |
| 103 | `se_horn_being_played.ogg` | `se_horn_being_played` | 角笛がブォーンと響き渡る音（演奏効果） | muse.c |
| 104 | `se_iron_ball_dragging_you.ogg` | `se_iron_ball_dragging_you` | 重い鉄球をズルズルと引きずられる音 | 予備（直接呼び出しなし） |
| 105 | `se_iron_ball_hits_you.ogg` | `se_iron_ball_hits_you` | 鉄球がドスンと身体に激突する音 | ball.c |
| 106 | `se_item_tumble_downwards.ogg` | `se_item_tumble_downwards` | アイテムが下層にカラカラ落ちていく音 | do.c |
| 107 | `se_jabberwock_burble.ogg` | `se_jabberwock_burble` | ジャバウォックのブツブツという不気味な声 | sounds.c |
| 108 | `se_kaablamm_of_mine.ogg` | `se_kaablamm_of_mine` | 地雷がカーブラムと大爆発する音 | trap.c |
| 109 | `se_kaboom.ogg` | `se_kaboom` | ドカーンという爆発音 | trap.c |
| 110 | `se_kaboom_boom_boom.ogg` | `se_kaboom_boom_boom` | ドカン、ドドーンという連続大爆発音 | timeout.c |
| 111 | `se_kaboom_door_explodes.ogg` | `se_kaboom_door_explodes` | ドアの罠がドカンと爆発する音 | lock.c |
| 112 | `se_kadoom_boulder_falls_in.ogg` | `se_kadoom_boulder_falls_in` | 巨石がカドームと穴に落ちる轟音 | dig.c |
| 113 | `se_kerplunk_boulder_gone.ogg` | `se_kerplunk_boulder_gone` | 巨石が水中や穴をカポーンと塞ぐ音 | hack.c |
| 114 | `se_kick_door_it_crashes_open.ogg` | `se_kick_door_it_crashes_open` | 蹴りによりドアがドカンと開く音 | dokick.c |
| 115 | `se_kick_door_it_shatters.ogg` | `se_kick_door_it_shatters` | 蹴りによりドアが粉々にバリーンと砕ける音 | dokick.c |
| 116 | `se_klick.ogg` | `se_klick` | カチッという小さな金属の作動音（解錠・機構） | lock.c |
| 117 | `se_klunk.ogg` | `se_klunk` | ゴトンという重い鈍い衝突音 | lock.c |
| 118 | `se_klunk_pipe.ogg` | `se_klunk_pipe` | 配管をゴトンと叩く衝撃音 | dokick.c |
| 119 | `se_laughter.ogg` | `se_laughter` | ハハハという笑い声（モンスターや魔法使用時） | sounds.c, uhitm.c |
| 120 | `se_lid_slams_open_falls_shut.ogg` | `se_lid_slams_open_falls_shut` | 箱の蓋がガタンと開き、ズンと閉まる音 | 予備（直接呼び出しなし） |
| 121 | `se_loud_click.ogg` | `se_loud_click` | カチャンという大きな機構の作動音（罠発動） | trap.c |
| 122 | `se_loud_crash.ogg` | `se_loud_crash` | 大音響の衝突・崩壊音 | dbridge.c, trap.c |
| 123 | `se_loud_pop.ogg` | `se_loud_pop` | ポーンと弾けて破裂する音（泡・爆発） | fountain.c |
| 124 | `se_loud_splash.ogg` | `se_loud_splash` | ドバシャーンという大きな水しぶき音 | dbridge.c |
| 125 | `se_low_buzzing.ogg` | `se_low_buzzing` | ブーンという低く響く羽音（蜂の巣・虫） | sounds.c |
| 126 | `se_low_hum.ogg` | `se_low_hum` | ブーンというハム・機械音（罠の予兆） | trap.c |
| 127 | `se_magic_whistle.ogg` | `se_magic_whistle` | 魔法の笛の澄んだ神秘的な音（ペットを呼び寄せる） | apply.c |
| 128 | `se_mana_drain.ogg` | `se_mana_drain` | 反魔法の罠や魔力吸収によるマナ枯渇音 | trap.c |
| 129 | `se_maniacal_laughter.ogg` | `se_maniacal_laughter` | ケタケタという狂気的な高笑い（巻物読後等） | read.c |
| 130 | `se_masticating_sound.ogg` | `se_masticating_sound` | クチャクチャと何かを噛む咀嚼音 | mon.c |
| 131 | `se_monster_behind_boulder.ogg` | `se_monster_behind_boulder` | 岩陰に隠れるモンスターの気配・物音 | hack.c |
| 132 | `se_mutter_imprecations.ogg` | `se_mutter_imprecations` | 呪いの言葉をブツブツと呟く声（店主の怒り） | shk.c |
| 133 | `se_mutter_incantation.ogg` | `se_mutter_incantation` | 呪文をブツブツと唱える声 | shk.c |
| 134 | `se_orc_grunt.ogg` | `se_orc_grunt` | オークのグアッという低い唸り声 | sounds.c |
| 135 | `se_paranoid_confirmation.ogg` | `se_paranoid_confirmation` | ソワソワした確認の囁き声 | 予備（直接呼び出しなし） |
| 136 | `se_polymorph.ogg` | `se_polymorph` | ポリモーフの罠による肉体変化・変身音 | trap.c |
| 137 | `se_potion_crash_and_break.ogg` | `se_potion_crash_and_break` | ポーション瓶がガシャンと割れ液体が散る音 | potion.c |
| 138 | `se_ring_in_drain.ogg` | `se_ring_in_drain` | 指輪が排水口にコロコロと転がり落ちる音 | do.c |
| 139 | `se_ripping_sound.ogg` | `se_ripping_sound` | ビリビリと布・紙が破れる音（装備破壊等） | worn.c |
| 140 | `se_roar.ogg` | `se_roar` | ガオーッという猛獣の咆哮 | trap.c |
| 141 | `se_rumbling.ogg` | `se_rumbling` | ゴゴゴという地鳴り音（罠・地震） | trap.c |
| 142 | `se_rumbling_of_earth.ogg` | `se_rumbling_of_earth` | ズズズという大地が轟く地鳴り | 予備（直接呼び出しなし） |
| 143 | `se_rushing_wind_noise.ogg` | `se_rushing_wind_noise` | ヒューという突風が吹き荒れる音 | mhitu.c |
| 144 | `se_rustling_paper.ogg` | `se_rustling_paper` | カサカサと紙・巻物が擦れ合う音 | apply.c |
| 145 | `se_sad_wailing.ogg` | `se_sad_wailing` | ウワーンという悲しげなすすり泣き・哀しみの声 | read.c, sounds.c |
| 146 | `se_sceptor_pounding.ogg` | `se_sceptor_pounding` | 笏（しゃく）で床をドンドンと叩く音（宮廷） | sounds.c |
| 147 | `se_scratching.ogg` | `se_scratching` | カリカリと爪で引っ掻く音（秘密の扉の探索） | do.c |
| 148 | `se_scream.ogg` | `se_scream` | ギャーッという悲鳴・絶叫（演奏効果等） | music.c |
| 149 | `se_screech.ogg` | `se_screech` | キーキーという甲高い叫び声 | 予備（直接呼び出しなし） |
| 150 | `se_sewer_song.ogg` | `se_sewer_song` | 下水道から聞こえる不気味な歌声（下水道の噴水） | fountain.c |
| 151 | `se_sharp_crack.ogg` | `se_sharp_crack` | バシッと鋭くひびが走る音 | 予備（直接呼び出しなし） |
| 152 | `se_shriek.ogg` | `se_shriek` | ギャーッという鋭い悲鳴（モンスターの声） | sounds.c |
| 153 | `se_shrill_whistle.ogg` | `se_shrill_whistle` | ピーッと甲高く突き刺さる笛の音（ホイッスル使用） | apply.c, mon.c |
| 154 | `se_sinister_laughter.ogg` | `se_sinister_laughter` | クックッという不敵・邪悪な笑い声（悪の存在） | eat.c |
| 155 | `se_sizzling.ogg` | `se_sizzling` | ジュージューと焼ける・溶ける音（酸・炎接触） | do.c |
| 156 | `se_slurping_sound.ogg` | `se_slurping_sound` | ズルズルとすする音（捕食・液体摂取） | mon.c |
| 157 | `se_smashing_and_crushing.ogg` | `se_smashing_and_crushing` | ガシャンと叩き潰す音（跳ね橋の可動） | dbridge.c |
| 158 | `se_snake_rattle.ogg` | `se_snake_rattle` | ガラガラヘビの尾のガラガラと鳴る警告音 | 予備（直接呼び出しなし） |
| 159 | `se_snakes_hissing.ogg` | `se_snakes_hissing` | ヘビのシャーという群体の威嚇音 | fountain.c |
| 160 | `se_snarl.ogg` | `se_snarl` | ウーッと牙をむいて低く唸る声 | sounds.c |
| 161 | `se_soft_click.ogg` | `se_soft_click` | カチッとかすかに鳴るいかにも怪しい音（罠感知） | trap.c |
| 162 | `se_soft_crackling.ogg` | `se_soft_crackling` | パチパチとかすかに燃える音（炎の魔法） | zap.c |
| 163 | `se_someone_bowling.ogg` | `se_someone_bowling` | ボウリングの球が転がるような音（転がる物体） | trap.c |
| 164 | `se_someone_searching.ogg` | `se_someone_searching` | ガサゴソと何かを捜索する音（探索中のモンスター） | sounds.c |
| 165 | `se_someone_summoning.ogg` | `se_someone_summoning` | ウォーという召喚の呪文を唱える声 | mcastu.c |
| 166 | `se_someone_yells.ogg` | `se_someone_yells` | 誰かが叫ぶ声（警衛の呼び声・緊急事態） | monmove.c |
| 167 | `se_splash.ogg` | `se_splash` | バシャッと水が跳ねる音（水中落下・投擲） | dbridge.c, do.c, dothrow.c, mthrowu.c |
| 168 | `se_splat_egg.ogg` | `se_splat_egg` | 卵がぐしゃっと潰れる音（投擲命中） | mthrowu.c |
| 169 | `se_splat_from_engulf.ogg` | `se_splat_from_engulf` | 丸呑みモンスターに消化されグシャッとなる音 | 予備（直接呼び出しなし） |
| 170 | `se_squawk.ogg` | `se_squawk` | ギャーギャーという鳥の鳴き声 | sounds.c |
| 171 | `se_squeak.ogg` | `se_squeak` | チューという動物の高い鳴き声・きしみ音 | sounds.c |
| 172 | `se_squeak_A.ogg` | `se_squeak_A` | きしみ床板の罠を踏んだときのきしみ音 — ラ音（A） | trap.c |
| 173 | `se_squeak_B.ogg` | `se_squeak_B` | きしみ床板の罠を踏んだときのきしみ音 — シ音（B） | trap.c |
| 174 | `se_squeak_B_flat.ogg` | `se_squeak_B_flat` | きしみ床板の罠を踏んだときのきしみ音 — シ♭音（B♭） | trap.c |
| 175 | `se_squeak_C.ogg` | `se_squeak_C` | きしみ床板の罠を踏んだときのきしみ音 — ド音（C） | trap.c |
| 176 | `se_squeak_D.ogg` | `se_squeak_D` | きしみ床板の罠を踏んだときのきしみ音 — レ音（D） | trap.c |
| 177 | `se_squeak_D_flat.ogg` | `se_squeak_D_flat` | きしみ床板の罠を踏んだときのきしみ音 — レ♭音（D♭） | trap.c |
| 178 | `se_squeak_E.ogg` | `se_squeak_E` | きしみ床板の罠を踏んだときのきしみ音 — ミ音（E） | trap.c |
| 179 | `se_squeak_E_flat.ogg` | `se_squeak_E_flat` | きしみ床板の罠を踏んだときのきしみ音 — ミ♭音（E♭） | trap.c |
| 180 | `se_squeak_F.ogg` | `se_squeak_F` | きしみ床板の罠を踏んだときのきしみ音 — ファ音（F） | trap.c |
| 181 | `se_squeak_F_sharp.ogg` | `se_squeak_F_sharp` | きしみ床板の罠を踏んだときのきしみ音 — ファ♯音（F♯） | trap.c |
| 182 | `se_squeak_G.ogg` | `se_squeak_G` | きしみ床板の罠を踏んだときのきしみ音 — ソ音（G） | trap.c |
| 183 | `se_squeak_G_sharp.ogg` | `se_squeak_G_sharp` | きしみ床板の罠を踏んだときのきしみ音 — ソ♯音（G♯） | trap.c |
| 184 | `se_squeal.ogg` | `se_squeal` | キーッという甲高い叫び声・きしみ | 予備（直接呼び出しなし） |
| 185 | `se_squelch.ogg` | `se_squelch` | グチャッという湿った踏みつけ音 | sit.c |
| 186 | `se_stone_breaking.ogg` | `se_stone_breaking` | バキッと石が割れる音（岩石魔法・爆発） | explode.c |
| 187 | `se_stone_crumbling.ogg` | `se_stone_crumbling` | ガラガラと石が崩れ落ちる音 | explode.c |
| 188 | `se_swoosh.ogg` | `se_swoosh` | ヒュッと素早く風を切る音（飛道具・魔法） | lock.c |
| 189 | `se_sword_blade_rings.ogg` | `se_sword_blade_rings` | 剣の刃がシャリーンと鳴る音（法具の認識） | apply.c |
| 190 | `se_teleport.ogg` | `se_teleport` | テレポートの罠による空間転移・瞬間移動音 | trap.c |
| 191 | `se_thud.ogg` | `se_thud` | ドスンという重い着地・衝突音 | worn.c |
| 192 | `se_thump.ogg` | `se_thump` | ドンという鈍い衝突音 | music.c |
| 193 | `se_thunderclap.ogg` | `se_thunderclap` | ドカーンと轟く雷鳴（祈り・神罰） | pray.c |
| 194 | `se_tumbler_click.ogg` | `se_tumbler_click` | 鍵穴のタンブラーがハマってカチッと鳴る音 | music.c |
| 195 | `se_typing_noise.ogg` | `se_typing_noise` | カタカタとタイプライターを打つような音（占術使用） | apply.c |
| 196 | `se_wail.ogg` | `se_wail` | ウオーッと泣き叫ぶ声（アンデッドや呪い） | 予備（直接呼び出しなし） |
| 197 | `se_wailing_of_the_banshee.ogg` | `se_wailing_of_the_banshee` | バンシーの死を予告する絶叫（バンシーの遭遇） | hack.c |
| 198 | `se_wall_of_force.ogg` | `se_wall_of_force` | ズズーンと重く響く力場・バリア音（魔法壁） | apply.c |
| 199 | `se_yelp.ogg` | `se_yelp` | キャンという短い悲鳴 | 予備（直接呼び出しなし） |
| 200 | `se_zap.ogg` | `se_zap` | ビビッという魔法・電撃の発射音 | muse.c |
| 201 | `se_zap_then_explosion.ogg` | `se_zap_then_explosion` | ビビッからドカンという魔法爆発音 | muse.c |
| 202 | `se_mon_chugging_potion.ogg` | `se_mon_chugging_potion` | モンスターがポーションをゴクゴクと飲む音 | muse.c |
| 203 | `se_combat_hit_slash.ogg` | `se_combat_hit_slash` | 刃物・刀剣・斧による斬撃ヒット音 | uhitm.c, mhitu.c, mhitm.c, dothrow.c |
| 204 | `se_combat_hit_blunt.ogg` | `se_combat_hit_blunt` | 鈍器・棍棒・石・打撃武器によるヒット音 | uhitm.c, mhitu.c, mhitm.c, dothrow.c |
| 205 | `se_combat_hit_pierce.ogg` | `se_combat_hit_pierce` | 槍・刺突武器・矢弾による貫通ヒット音 | uhitm.c, mhitu.c, mhitm.c, dothrow.c |
| 206 | `se_combat_hit_unarmed.ogg` | `se_combat_hit_unarmed` | 素手格闘・パンチ・キックによる打撃ヒット音 | uhitm.c, mhitu.c, mhitm.c |
| 207 | `se_combat_hit_whip.ogg` | `se_combat_hit_whip` | 鞭・濡れたタオルによる攻撃命中音 | uhitm.c, mhitu.c, mhitm.c |
| 208 | `se_combat_hit_ironball.ogg` | `se_combat_hit_ironball` | 鉄球・鉄鎖による重金属・鎖衝突音 | uhitm.c, mhitu.c, mhitm.c |
| 209 | `se_combat_hit_shield.ogg` | `se_combat_hit_shield` | 盾による重い防具シールドバッシュ音（近接限定） | uhitm.c, mhitu.c, mhitm.c |
| 210 | `se_combat_hit_corpse.ogg` | `se_combat_hit_corpse` | 死体・肉塊武器による生体打撃音 | uhitm.c, mhitu.c, mhitm.c |
| 211 | `se_combat_hit_pick.ogg` | `se_combat_hit_pick` | つるはし・マトックによる硬質な採掘具打撃音 | uhitm.c, mhitu.c, mhitm.c |
| 212 | `se_combat_hit_wand.ogg` | `se_combat_hit_wand` | 杖・ロッドによる硬く乾いた小打撃音 | uhitm.c, mhitu.c, mhitm.c |
| 213 | `se_combat_hit_other.ogg` | `se_combat_hit_other` | 未分類アイテム（本・巻物・薬・食料・宝石等）全般の汎用フォールバック打撃音 | uhitm.c, mhitu.c, mhitm.c, dothrow.c, mthrowu.c |
| 214 | `se_combat_miss.ogg` | `se_combat_miss` | 攻撃の空振り音（風切り音） | uhitm.c, mhitu.c, mhitm.c |
| 215 | `se_combat_shoot_bow.ogg` | `se_combat_shoot_bow` | 弓矢の発射音（弦を弾く音） | dothrow.c, mthrowu.c |
| 216 | `se_combat_shoot_crossbow.ogg` | `se_combat_shoot_crossbow` | クロスボウの発射音（バネ解放音） | dothrow.c, mthrowu.c |
| 217 | `se_combat_shoot_sling.ogg` | `se_combat_shoot_sling` | スリング（投石器）の旋回・射出音 | dothrow.c, mthrowu.c |
| 218 | `se_combat_throw.ogg` | `se_combat_throw` | 手投げによる投擲音 | dothrow.c, mthrowu.c |
| 219 | `se_combat_throw_boomerang.ogg` | `se_combat_throw_boomerang` | ブーメランの旋回投擲音 | dothrow.c |
| 220 | `se_combat_throw_mjollnir.ogg` | `se_combat_throw_mjollnir` | ミョルニルの電撃を帯びた豪快な投擲音 | dothrow.c |
| 221 | `se_combat_miss_thud.ogg` | `se_combat_miss_thud` | 飛翔体が外れて壁や床に激突する音 | dothrow.c, mthrowu.c |
| 222 | `se_combat_spell_cast.ogg` | `se_combat_spell_cast` | 呪文の詠唱・魔法発動音 | spell.c, mcastu.c |
| 223 | `se_combat_wand_zap.ogg` | `se_combat_wand_zap` | 杖を振った際の発動音 | zap.c, muse.c |
| 224 | `se_mon_claw.ogg` | `se_mon_claw` | モンスターの爪による引き裂き音 | mhitu.c, mhitm.c |
| 225 | `se_mon_bite.ogg` | `se_mon_bite` | モンスターの噛みつき音 | mhitu.c, mhitm.c |
| 226 | `se_mon_sting.ogg` | `se_mon_sting` | モンスターの毒針・刺突音 | mhitu.c, mhitm.c |
| 227 | `se_mon_butt.ogg` | `se_mon_butt` | モンスターの角・頭突き衝突音 | mhitu.c, mhitm.c |
| 228 | `se_mon_touch.ogg` | `se_mon_touch` | モンスターの接触・麻痺音 | mhitu.c, mhitm.c |
| 229 | `se_mon_tentacle.ogg` | `se_mon_tentacle` | 触手による攻撃・絡みつき音 | mhitu.c, mhitm.c |
| 230 | `se_mon_kick.ogg` | `se_mon_kick` | モンスターの蹴り・踏みつけ音 | mhitu.c, mhitm.c |
| 231 | `se_mon_hug.ogg` | `se_mon_hug` | 締めつけ・怪力による抱き締め音 | mhitu.c, mhitm.c |
| 232 | `se_mon_gaze.ogg` | `se_mon_gaze` | 凝視・魔眼による視線攻撃音 | mhitu.c, mhitm.c |
| 233 | `se_mon_engulf.ogg` | `se_mon_engulf` | 丸呑み・呑み込み音 | mhitu.c, mhitm.c |
| 234 | `se_mon_breath.ogg` | `se_mon_breath` | ドラゴン等のブレス放出音 | mhitu.c, mhitm.c |
| 235 | `se_mon_spit.ogg` | `se_mon_spit` | 毒液・酸の吐出音 | mhitu.c, mhitm.c |
...
| L1054 | `Soundeffect` | `se_klunk, 50` | `se_klunk.ogg` | `Soundeffect(se_klunk, 50);` |
| L1068 | `Soundeffect` | `se_klick, 50` | `se_klick.ogg` | `Soundeffect(se_klick, 50);` |
| L1135 | `Soundeffect` | `se_swoosh, 25` | `se_swoosh.ogg` | `Soundeffect(se_swoosh, 25);` |
| L1210 | `Soundeffect` | `se_kaboom_door_explodes, 75` | `se_kaboom_door_explodes.ogg` | `Soundeffect(se_kaboom_door_explodes, 75);` |
| L1214 | `Soundeffect` | `se_explosion, 75` | `se_explosion.ogg` | `Soundeffect(se_explosion, 75);` |
| L1232 | `Soundeffect` | `se_crashing_sound, 100` | `se_crashing_sound.ogg` | `Soundeffect(se_crashing_sound, 100);` |
