...
| 14 | `se_bees.ogg` | `se_bees` | 蜂の群体がブンブン群がる羽音 | sounds.c |
| 15 | `se_blast.ogg` | `se_blast` | 爆発・爆風音（魔法爆発・罠爆発） | explode.c |
| 16 | `se_board_squeak.ogg` | `se_board_squeak` | 床板がギシッときしむ音（きしみ床トラップ） | 予備（直接呼び出しなし） |
| 17 | `se_board_squeaks_loudly.ogg` | `se_board_squeaks_loudly` | 床板がギシギシと大きくきしむ音 | 予備（直接呼び出しなし） |
| 18 | `se_boing.ogg` | `se_boing` | ビヨーンと跳ねる音（ゴムや跳躍） | muse.c |
| 19 | `se_bolt_of_lightning.ogg` | `se_bolt_of_lightning` | ビリビリと落雷・雷撃が走る音 | mcastu.c |
| 20 | `se_bone_rattle.ogg` | `se_bone_rattle` | 骨がカタカタと鳴る音（スケルトンやアンデッド） | sounds.c |
| 21 | `se_boomerang_klonk.ogg` | `se_boomerang_klonk` | ブーメランが何かに当たって跳ね返る音 | zap.c |
| 22 | `se_boulder_drop.ogg` | `se_boulder_drop` | 巨石がドスンと落ちる音（階段や穴への落下） | do.c |
| 23 | `se_bovine_bellow.ogg` | `se_bovine_bellow` | 牛・ウシの低い唸り声 | 予備（直接呼び出しなし） |
| 24 | `se_bovine_moo.ogg` | `se_bovine_moo` | 牛のモーという鳴き声 | sounds.c |
| 25 | `se_bubble_rising.ogg` | `se_bubble_rising` | 水中からポコポコと泡が湧き立つ音 | fountain.c |
| 26 | `se_bugle_playing_reveille.ogg` | `se_bugle_playing_reveille` | ラッパで起床ラッパを演奏する音 | muse.c |
| 27 | `se_buzz.ogg` | `se_buzz` | ブーンという低周波の羽音・電気音 | sounds.c |
| 28 | `se_canine_bark.ogg` | `se_canine_bark` | 犬・狼のワンワンという吠え声 | 予備（直接呼び出しなし） |
| 29 | `se_canine_growl.ogg` | `se_canine_growl` | 犬・狼のウーンという唸り声 | 予備（直接呼び出しなし） |
| 30 | `se_canine_howl.ogg` | `se_canine_howl` | 犬・狼の遠吠え（アオーンという声） | were.c |
| 31 | `se_canine_whine.ogg` | `se_canine_whine` | 犬のクーン・キュンという甘え声 | 予備（直接呼び出しなし） |
| 32 | `se_canine_yelp.ogg` | `se_canine_yelp` | 犬のキャンという痛みの悲鳴 | 予備（直接呼び出しなし） |
| 33 | `se_canine_yip.ogg` | `se_canine_yip` | 犬の甲高いキュッという鳴き声 | 予備（直接呼び出しなし） |
| 34 | `se_canine_yowl.ogg` | `se_canine_yowl` | 犬の長く引き伸ばした吠え声 | 予備（直接呼び出しなし） |
| 35 | `se_chain_shatters.ogg` | `se_chain_shatters` | 鎖がバキッと砕け散る音（魔法爆発等で） | explode.c |
| 36 | `se_chains_rattling_gears_turning.ogg` | `se_chains_rattling_gears_turning` | 鎖がジャラジャラ鳴りながらギアが回る音（跳ね橋の可動） | dbridge.c |
| 37 | `se_chant.ogg` | `se_chant` | 呪文の詠唱・宗教的な賛歌の声 | 予備（直接呼び出しなし） |
| 38 | `se_chirp.ogg` | `se_chirp` | 小鳥・小動物のちゅんちゅんという鳴き声 | sounds.c |
| 39 | `se_clanging_sound.ogg` | `se_clanging_sound` | 金属がカンカンと大きく鳴り響く音 | 予備（直接呼び出しなし） |
| 40 | `se_clank.ogg` | `se_clank` | カンという短い金属衝突音 | worn.c |
| 41 | `se_clanking_pipe.ogg` | `se_clanking_pipe` | 配管がカンカンと響く音（噴水の配管操作） | fountain.c |
| 42 | `se_clash.ogg` | `se_clash` | 武器同士が激しくぶつかり合う金属音 | dig.c |
| 43 | `se_cockatrice_hiss.ogg` | `se_cockatrice_hiss` | コカトリスの石化の威嚇音 | uhitm.c |
| 44 | `se_cough.ogg` | `se_cough` | ゲホゲホと咳き込む声（毒・粉塵吸入） | mthrowu.c |
| 45 | `se_courtly_conversation.ogg` | `se_courtly_conversation` | 宮廷部屋のにぎやかな会話音（謁見室） | sounds.c |
| 46 | `se_cracking_sound.ogg` | `se_cracking_sound` | パキッとひびが入る音（防具・呪いの解除等） | worn.c |
| 47 | `se_crackling.ogg` | `se_crackling` | パチパチとはじける音（炎・電気） | 予備（直接呼び出しなし） |
| 48 | `se_crackling_of_hellfire.ogg` | `se_crackling_of_hellfire` | 地獄の業火がメラメラと燃えさかる音（呪われた道具） | apply.c |
| 49 | `se_crash.ogg` | `se_crash` | ドカンと激しく崩壊・衝突する音 | 予備（直接呼び出しなし） |
| 50 | `se_crash_door.ogg` | `se_crash_door` | ドアがドカンと破壊される音（強制突破） | dokick.c |
| 51 | `se_crash_something_broke.ogg` | `se_crash_something_broke` | 何かがガシャンと割れて壊れる音 | 予備（直接呼び出しなし） |
| 52 | `se_crash_throne_destroyed.ogg` | `se_crash_throne_destroyed` | 玉座が蹴り倒されて粉々に砕ける大音響 | dokick.c |
| 53 | `se_crash_through_floor.ogg` | `se_crash_through_floor` | 床を突き破って落とし穴に落ちる音 | muse.c |
| 54 | `se_crashed_ceiling.ogg` | `se_crashed_ceiling` | 天井が崩落してガラガラ落ちる音 | 予備（直接呼び出しなし） |
| 55 | `se_crashing_boulder.ogg` | `se_crashing_boulder` | 巨石がドカドカと激しく転がり激突する音 | do.c |
| 56 | `se_crashing_rock.ogg` | `se_crashing_rock` | 岩石がガラガラと崩れ落ちる音（掘削・崩壊） | dig.c |
| 57 | `se_crashing_sound.ogg` | `se_crashing_sound` | 激しい衝突・破壊音（ドアや施錠装置の破壊） | lock.c |
| 58 | `se_croc_bellow.ogg` | `se_croc_bellow` | ワニの低く響く咆哮 | 予備（直接呼び出しなし） |
| 59 | `se_crumbling_sound.ogg` | `se_crumbling_sound` | 砂岩や壁がボロボロと崩れる音（魔法・掘削） | zap.c |
| 60 | `se_crunching_sound.ogg` | `se_crunching_sound` | バリバリと硬いものを噛み砕く音（捕食・踏み砕き） | mon.c |
| 61 | `se_crushing_sound.ogg` | `se_crushing_sound` | メキメキと挟まれて押し潰される音（跳ね橋） | dbridge.c |
| 62 | `se_deafening_roar_atmospheric.ogg` | `se_deafening_roar_atmospheric` | 耳をつんざく轟音（強力な罠・爆発の雰囲気） | trap.c |
| 63 | `se_destroy_web.ogg` | `se_destroy_web` | クモの巣をビリっと破り払う音 | ball.c |
| 64 | `se_distant_thunder.ogg` | `se_distant_thunder` | 遠くに聞こえる雷鳴（嵐・スクロール効果） | mon.c |
| 65 | `se_divine_music.ogg` | `se_divine_music` | 神聖な天上の音楽（神への祈り成功時） | pray.c |
| 66 | `se_door_crash_open.ogg` | `se_door_crash_open` | ドアがドカンと蹴り開けられる音（モンスターの侵入） | monmove.c |
| 67 | `se_door_open.ogg` | `se_door_open` | ドアがギィッと開く音（通常の開扉） | monmove.c |
| 68 | `se_door_unlock_and_open.ogg` | `se_door_unlock_and_open` | 鍵がカチャリと解錠されてドアが開く音 | monmove.c |
| 69 | `se_drain_noises.ogg` | `se_drain_noises` | 排水口がゴボゴボと鳴る音（排水溝近く） | do.c |
| 70 | `se_dry_throat_rattle.ogg` | `se_dry_throat_rattle` | 乾燥した喉がガラガラ鳴る音（モンスターの息） | mthrowu.c |
| 71 | `se_egg_cracking.ogg` | `se_egg_cracking` | 卵の殻がパキッと割れる音（蹴った時） | dokick.c |
| 72 | `se_egg_splatting.ogg` | `se_egg_splatting` | 卵がグシャッと踏み潰れる音 | dokick.c |
| 73 | `se_elephant_trumpet.ogg` | `se_elephant_trumpet` | 象のパオーンという高らかな鳴き声 | sounds.c |
| 74 | `se_equine_neigh.ogg` | `se_equine_neigh` | 馬のヒヒーンという高いいななき | sounds.c |
| 75 | `se_equine_whicker.ogg` | `se_equine_whicker` | 馬の低く柔らかいいななき | sounds.c |
| 76 | `se_equine_whinny.ogg` | `se_equine_whinny` | 馬の短いいななき声 | sounds.c |
| 77 | `se_explosion.ogg` | `se_explosion` | 大爆発音（錠前爆発・爆破罠） | lock.c |
| 78 | `se_faint_chime.ogg` | `se_faint_chime` | かすかなチリンというチャイムの音（魔法感知） | spell.c |
| 79 | `se_faint_sloshing.ogg` | `se_faint_sloshing` | かすかな水が揺れる音（アイテム生成時） | mkobj.c |
| 80 | `se_faint_splashing.ogg` | `se_faint_splashing` | かすかな水しぶき音（ホイールへの接触） | apply.c |
| 81 | `se_feline_meow.ogg` | `se_feline_meow` | 猫のニャーという鳴き声 | sounds.c |
| 82 | `se_feline_mew.ogg` | `se_feline_mew` | 子猫のミューという細い鳴き声 | sounds.c |
| 83 | `se_feline_purr.ogg` | `se_feline_purr` | 猫のゴロゴロと喉を鳴らす音 | sounds.c |
| 84 | `se_feline_yelp.ogg` | `se_feline_yelp` | 猫のギャッという悲鳴 | 予備（直接呼び出しなし） |
| 85 | `se_feline_yip.ogg` | `se_feline_yip` | 猫の甲高いキーという鳴き声 | 予備（直接呼び出しなし） |
| 86 | `se_feline_yowl.ogg` | `se_feline_yowl` | 猫のウウウという低い唸り声 | sounds.c |
| 87 | `se_furious_bubbling.ogg` | `se_furious_bubbling` | 液体が激しくボコボコと泡立つ音（噴水の呪い） | fountain.c |
| 88 | `se_gear_turn.ogg` | `se_gear_turn` | 歯車がカチリと一段回る音（機構の動作） | music.c |
| 89 | `se_gears_turning_chains_rattling.ogg` | `se_gears_turning_chains_rattling` | 歯車が回り鎖が引き上げられる音（跳ね橋の引き上げ） | dbridge.c |
| 90 | `se_glass_crashing.ogg` | `se_glass_crashing` | ガラスが激しくバリーンと割れる音 | dokick.c |
| 91 | `se_glass_shattering.ogg` | `se_glass_shattering` | ガラス瓶が粉々にパリーンと砕け散る音 | dokick.c |
| 92 | `se_groan.ogg` | `se_groan` | ウゥーといううめき声（モンスターや地形） | sounds.c |
| 93 | `se_groans_and_moans.ogg` | `se_groans_and_moans` | 呻きとうめき声（アンデッドの気配・死体部屋） | do.c |
| 94 | `se_growl.ogg` | `se_growl` | ウーンという低い唸り声 | 予備（直接呼び出しなし） |
| 95 | `se_grunt.ogg` | `se_grunt` | フンと鼻を鳴らす短い声 | sounds.c |
| 96 | `se_guards_footsteps.ogg` | `se_guards_footsteps` | 警備員の重い足音が近づく音（金庫室侵入） | sounds.c |
| 97 | `se_gurgle.ogg` | `se_gurgle` | ゴボゴボと湧き立つ液体の音（モンスターの声） | sounds.c |
| 98 | `se_gushing_sound.ogg` | `se_gushing_sound` | ドバーッと液体や水が噴き出す音（配管破壊） | dokick.c |
| 99 | `se_heart_beat.ogg` | `se_heart_beat` | トックン、トックンという心臓の鼓動音（聴診器使用） | apply.c |
| 100 | `se_hiss.ogg` | `se_hiss` | シュッというヘビや蒸気の威嚇音 | sounds.c |
| 101 | `se_hollow_sound.ogg` | `se_hollow_sound` | コンコンと中が空洞に響く音（壁や箱の確認） | apply.c |
| 102 | `se_horn_being_played.ogg` | `se_horn_being_played` | 角笛がブォーンと響き渡る音（演奏効果） | muse.c |
| 103 | `se_iron_ball_dragging_you.ogg` | `se_iron_ball_dragging_you` | 重い鉄球をズルズルと引きずられる音 | 予備（直接呼び出しなし） |
| 104 | `se_iron_ball_hits_you.ogg` | `se_iron_ball_hits_you` | 鉄球がドスンと身体に激突する音 | ball.c |
| 105 | `se_item_tumble_downwards.ogg` | `se_item_tumble_downwards` | アイテムが下層にカラカラ落ちていく音 | do.c |
| 106 | `se_jabberwock_burble.ogg` | `se_jabberwock_burble` | ジャバウォックのブツブツという不気味な声 | sounds.c |
| 107 | `se_kaablamm_of_mine.ogg` | `se_kaablamm_of_mine` | 地雷がカーブラムと大爆発する音 | trap.c |
| 108 | `se_kaboom.ogg` | `se_kaboom` | ドカーンという爆発音 | trap.c |
| 109 | `se_kaboom_boom_boom.ogg` | `se_kaboom_boom_boom` | ドカン、ドドーンという連続大爆発音 | timeout.c |
| 110 | `se_kaboom_door_explodes.ogg` | `se_kaboom_door_explodes` | ドアの罠がドカンと爆発する音 | lock.c |
| 111 | `se_kadoom_boulder_falls_in.ogg` | `se_kadoom_boulder_falls_in` | 巨石がカドームと穴に落ちる轟音 | dig.c |
| 112 | `se_kerplunk_boulder_gone.ogg` | `se_kerplunk_boulder_gone` | 巨石が水中や穴をカポーンと塞ぐ音 | hack.c |
| 113 | `se_kick_door_it_crashes_open.ogg` | `se_kick_door_it_crashes_open` | 蹴りによりドアがドカンと開く音 | dokick.c |
| 114 | `se_kick_door_it_shatters.ogg` | `se_kick_door_it_shatters` | 蹴りによりドアが粉々にバリーンと砕ける音 | dokick.c |
| 115 | `se_klick.ogg` | `se_klick` | カチッという小さな金属の作動音（解錠・機構） | lock.c |
| 116 | `se_klunk.ogg` | `se_klunk` | ゴトンという重い鈍い衝突音 | lock.c |
| 117 | `se_klunk_pipe.ogg` | `se_klunk_pipe` | 配管をゴトンと叩く衝撃音 | dokick.c |
| 118 | `se_laughter.ogg` | `se_laughter` | ハハハという笑い声（モンスターや魔法使用時） | sounds.c, uhitm.c |
| 119 | `se_lid_slams_open_falls_shut.ogg` | `se_lid_slams_open_falls_shut` | 箱の蓋がガタンと開き、ズンと閉まる音 | 予備（直接呼び出しなし） |
| 120 | `se_loud_click.ogg` | `se_loud_click` | カチャンという大きな機構の作動音（罠発動） | trap.c |
| 121 | `se_loud_crash.ogg` | `se_loud_crash` | 大音響の衝突・崩壊音 | dbridge.c, trap.c |
| 122 | `se_loud_pop.ogg` | `se_loud_pop` | ポーンと弾けて破裂する音（泡・爆発） | fountain.c |
...
| L1291 | `Soundeffect` | `se_soft_click, 30` | `se_soft_click.ogg` | `Soundeffect(se_soft_click, 30);` |
| L1458 | `Soundeffect` | `tsnds[trap->tnote], 50` | `se_unknown.ogg` | `Soundeffect(tsnds[trap->tnote], 50);` |
| L1475 | `Soundeffect` | `tsnds[trap->tnote], 50` | `se_unknown.ogg` | `Soundeffect(tsnds[trap->tnote], 50);` |
| L1567 | `Soundeffect` | `se_roar, 100` | `se_roar.ogg` | `Soundeffect(se_roar, 100);` |
| L2247 | `Soundeffect` | `se_roar, 60` | `se_roar.ogg` | `Soundeffect(se_roar, 60);` |
| L2588 | `Soundeffect` | `se_kaablamm_of_mine, 80` | `se_kaablamm_of_mine.ogg` | `Soundeffect(se_kaablamm_of_mine, 80);` |
| L3348 | `Soundeffect` | `se_someone_bowling, 60` | `se_someone_bowling.ogg` | `Soundeffect(se_someone_bowling, 60);` |
| L3351 | `Soundeffect` | `se_rumbling, 60` | `se_rumbling.ogg` | `Soundeffect(se_rumbling, 60);` |
| L3545 | `Soundeffect` | `se_loud_crash, 80` | `se_loud_crash.ogg` | `Soundeffect(se_loud_crash, 80);` |
| L4376 | `Soundeffect` | `se_deafening_roar_atmospheric, 100` | `se_deafening_roar_atmospheric.ogg` | `Soundeffect(se_deafening_roar_atmospheric, 100);` |
| L4397 | `Soundeffect` | `se_low_hum, 100` | `se_low_hum.ogg` | `Soundeffect(se_low_hum, 100);` |
| L6738 | `Soundeffect` | `se_kaboom, 80` | `se_kaboom.ogg` | `Soundeffect(se_kaboom, 80);` |
