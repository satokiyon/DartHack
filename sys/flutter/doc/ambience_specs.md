...
graph TD
    subgraph Layer1["【レイヤー1】BGM層（排他・クロスフェード）"]
        FloorBGM["フロア全体BGM<br>（運命の大迷宮、ノーム鉱山、ゲヘナ等）"]
        RoomBGM["特別な部屋専用BGM（ルームBGM）<br>（全特別室・テーマ部屋：王座、寺院、蜂の巣、蜘蛛の巣等）"]
        FloorBGM <==>|"部屋進入・退出時に<br>約300ms〜400msでクロスフェード<br>（一時停止 ⇔ 続きから再開）"| RoomBGM
    end
    ...
    subgraph Layer3["【レイヤー3】効果音・音声層（ワンショット重ね合わせ）"]
        SEVoice["SE・ボイス<br>（店主の怒号、攻撃音、呪文詠唱、足音等）"]
    end
...
1. **特別な部屋・テーマ部屋（全室ルームBGMクロスフェード）**:
   ...
   - **進入時**: 現在のフロアBGMを約300ms〜400msでフェードアウトして**一時停止（Pause）**し、その部屋専用のBGMをスムーズにフェードイン再生する。
   - **退出時**: ルームBGMを約300ms〜400msでフェードアウト停止し、退避していたフロアBGMを**一時停止位置からフェードイン復帰（Resume）**する。
   - **メリット**: 「特別な部屋に入った」というゲーム演出が明確になり、フロアBGMとの音の濁りや喧嘩を完全に防止する。
2. **地形・天候の環境音（フロアBGMとの同時再生）**:
...
3. **効果音・音声（SEプール多重再生）**:
   - モンスター攻撃、足音、呪文、店主の怒号ボイス（`voice_shk`）などは、8チャンネルのSEプールから即座にワンショット再生され、BGMや環境音の上に重ねてクリアに出力される。
...
1. **進入時（`ambience_begin` 受信）**:
   - 現在再生中のフロアBGMを記憶（`_savedFloorBgm`）。
   - 約300ms〜400ms（ステップ約50ms×7段階）でフロアBGMの音量をゼロへフェードアウトし、`_bgmPlayer.pause()` で**再生位置を保持したまま一時停止**。
   - ルームBGMのソースをロードし、音量ゼロから300ms〜400msで目標音量へ**フェードイン再生**。
2. **退出時（`ambience_end` 受信）**:
   - ルームBGMを約300ms〜400msでフェードアウトして停止。
   - 退避していたフロアBGM（`_savedFloorBgm`）が存在する場合、`_bgmPlayer.resume()` を呼び出し、音量ゼロから300ms〜400msで元の音量へ**フェードイン復帰**。
   - 退避変数をクリア（`_savedFloorBgm = null`）。

### 4.3 地形・天候環境音とSEの同時ミキシング設計
...
1. **距離減衰（`proximity`）のリアルタイム反映**:
   ...
   - 基準音量や減衰カーブは、実機でのプレイ・動作確認を行いながら耳触りの良さを検証して調整する。
2. **ワンショットSE・ボイスとの多重再生**:
   - 店主が万引きを咎めて罵声を浴びせるイベント（`voice_shk` / `voice_speech`）や各種アクションSEは、独立した SE プール（`_pool`）から即座にワンショット再生される。
   - 独立した別チャネルで再生されるため、環境音やBGMが途切れたり、操作や表示にタイムラグが生じることは一切ない。
