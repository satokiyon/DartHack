...
本ツールは、『[sound_macros_list.md](../../doc/sound_macros_list.md)』および『[combat_sound_specification.md](../../doc/combat_sound_specification.md)』に定義されている **全 300 種類の効果音・戦闘アクション音・実績音・楽器演奏音・声音** を、効率よく・高品質かつ狙い通りに収集・選定・正規化・管理するためのローカルWebワークスペースです。
...
1. **進捗ダッシュボード & カテゴリフィルタ**:
   - 全300音の収集進捗率（パーセンテージ）をリアルタイム表示。
   - 「未設定のみ」「確定済のみ」「⚔️ 戦闘(26)」「効果音(197)」「実績(23)」「楽器(47)」「声音(7)」による瞬時絞り込み。
   - サウンドIDや日本語説明文、推奨検索キーワードによるインクリメンタル検索。
2. **⚔️ 戦闘アクション効果音（26種）特化キュレーション & 自動音響処理**:
   - **4つのサブカテゴリ絞り込み**: 「🗡️ 近接攻撃 (5)」「🏹 遠隔・投擲 (7)」「✨ 魔法攻撃 (2)」「🐾 モンスター固有 (12)」に瞬時に切り替え可能。
   - **特化検索キーワード**: 全26音それぞれに割り当てられた日英特化キーワード（「斬撃 剣」「打撃 鈍器」「弓 矢」「モンスター 爪」等 / 「sword slash」「blunt hit」「bow arrow shot」「monster claw」等）により、13の音源サイトで的確な検索結果を即座に表示。
...
### 方法 A: ⚔️ 戦闘アクション効果音キュレーター（戦闘26音に特化）
```powershell
