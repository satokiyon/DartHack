...
static int palette[CLR_MAX] = {
	0xFF546E7A,	// CLR_BLACK
	0xFFFF5252,	// CLR_RED
	0xFF38D9A9,	// CLR_GREEN
	0xFFBA7C59, // CLR_BROWN
	0xFF42A5F5,	// CLR_BLUE
	0xFFBA68C8,	// CLR_MAGENTA
	0xFF26C6DA,	// CLR_CYAN
	0xFFCFD8DC,	// CLR_GRAY
	0xFFECEFF1,	// NO_COLOR
	0xFFFFA726,	// CLR_ORANGE
	0xFF69F0AE,	// CLR_BRIGHT_GREEN
	0xFFFFEE58,	// CLR_YELLOW
	0xFF80D8FF,	// CLR_BRIGHT_BLUE
	0xFFFF4081,	// CLR_BRIGHT_MAGENTA
	0xFF84FFFF,	// CLR_BRIGHT_CYAN
...
static void flutter_askname(void) {
    ...
    while (saves && saves[idx]) {
        ...
        if (idx > 0) {
            strncat(saves_buf, ";", sizeof(saves_buf) - strlen(saves_buf) - 1);
        }
        strncat(saves_buf, saves[idx], sizeof(saves_buf) - strlen(saves_buf) - 1);
        idx++;
    }
    ...
    if (g_askname_mode != 2) {
        ...
        const char *prefix_jp = "[自動セーブ] ";
        const char *prefix_en = "[Autosave] ";
        size_t len_jp = strlen(prefix_jp);
        size_t len_en = strlen(prefix_en);

        if (strncmp(name_ptr, prefix_jp, len_jp) == 0) {
            name_ptr += len_jp;
        } else if (strncmp(name_ptr, prefix_en, len_en) == 0) {
            name_ptr += len_en;
        }
