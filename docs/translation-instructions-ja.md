...
- `src/pline.c` のメッセージ表示関数（`You`, `Your`, `You_feel`, `You_hear`, `You_see`, `You_cant`, `There`, `pline_The`, `verbalize`, `custompline` など）は、関数本体だけを見て訳さない。呼び出し(参照)先の文字列と結合した最終表示文で自然さを確認する。
- とくに接頭辞（例: 「あなたは」「あなたには」「その」）を変更した場合は、呼び出し側で語順・助詞・時制が破綻しないかを確認し、必要なら呼び出し側リテラルを最小限調整する。
- `You_hear(...)` は接頭辞「あなたには」（水中・夢中時は別）を自動で付与するため、呼び出し側リテラルで主語を重複させない。自然さ確認は必ず最終表示文で行う。
- `You_feel(...)` は接頭辞「あなたは」（夢中時は「あなたは夢の中で」）を自動で付与するため、呼び出し側リテラルで主語を重複させない。自然さ確認は必ず最終表示文で行う。
- `You_see(...)` は接頭辞「あなたは」を自動で付与し、状況によって「あなたは気配で」「あなたは夢の中で」に切り替わる。呼び出し側リテラルはこの前提で語順を調整する。
- `隠し%s` のようなテンプレートは `%s` 展開後（例: `扉`, `通路`）の連結結果を確認し、語として不自然になっていないかをチェックする。
