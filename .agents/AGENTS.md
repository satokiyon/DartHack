<!-- Modified by NetHackJP contributor @satokiyon; latest change date: 2026-07-08. -->
...
- **対策**:
  1. C側から FFI 経由で文字列を渡す場合、即時解放は行わず、スレッドセーフな静的リングバッファ（例: 16面バッファ、各4096バイト）を経由させることで、Dart 側が安全にメモリをコピーし終えるまで生存期間を保持させてください。
