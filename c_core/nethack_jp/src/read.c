...
doread(void)
{
    ...
    if (otyp != SCR_BLANK_PAPER) {
        ...
        if (Blind)
            pline(nodisappear
                      ? (silently ? "巻物の呪文を心の中で思い描いた."
                                  : "巻物の呪文を唱えた.")
                      : (silently
                             ? "巻物の呪文を心の中で思い描くと、巻物は消えた."
                             : "巻物の呪文を唱えると、巻物は消えた."));
        else
            pline(nodisappear ? "巻物を読んだ."
                              : "巻物を読むと、巻物は消えた.");
        if (confused) {
            if (Hallucination)
                pline("あまりに幻覚が強くて、うまく読めなかった...");
            else
                pline(silently
                          ? "混乱していたため、魔法の言葉を取り違えてしまった..."
                          : "混乱していたため、魔法の言葉を言い間違えてしまった...");
        }
    }
    if (!seffects(scroll)) {
...
seffect_mail(struct obj **sobjp)
{
    ...
    case 1:
        /* scroll of mail obtained from bones file or from wishing;
           ...
           mail if you win! */
          pline(odd ? "これは幸運が失われると脅すチェーンレターのようだ."
                : "これはラルンの眼を見つけた者宛ての迷惑郵便のようだ.");
        break;
    default:
#ifdef MAIL
        readmail(sobj);
...
do_class_genocide(void)
{
    ...
    for (j = 0; ; j++) {
        ...
        if (!goodcnt && class != mons[gu.urole.mnum].mlet
            ...
            if (gonecnt)
                pline("その種の怪物はすでにすべていなくなっている.");
            else if (immunecnt || class == S_invisible)
                You("そのようなモンスターを虐殺することは許されなかった.");
...
do_stinking_cloud(struct obj *sobj, boolean mention_stinking)
{
    ...
    pline(mention_stinking ? "悪臭雲の中心をどこにする?"
                           : "雲の中心をどこにする?");
    cc.x = u.ux;
    cc.y = u.uy;
    ...
    if (getpos(&cc, TRUE, "目的の場所") < 0) {
        ...
        if (Hallucination)
            pline("うっ... 誰かがとんでもない臭いを放った.");
        else
            pline(sobj->oclass == SCROLL_CLASS
                      ? "巻物は腐った卵のような臭いを放ちながら崩れた."
                      : "腐った卵のような臭いがした.");
        return;
    }
