    ...
    boolean costly)
{
...

    {
        ...
        if (!u.usteed) {
            ...
            if (givemesg)
                pline("%s力で%sを動かした.",
                      easypush ? "わずかな" : "大きな", what);
            if (!easypush)
                exercise(A_STR, TRUE);
        } else {
            if (givemesg)
                pline("%sが%sを動かした.", YMonnam(u.usteed), what);
        }
...
moverock_core(coordxy sx, coordxy sy)
{
    ...
    while ((otmp = sobj_at(BOULDER, sx, sy)) != 0) {
        ...
        if (verysmall(gy.youmonst.data) && !u.usteed) {
            if (Blind)
                feel_location(sx, sy);
            pline("小さすぎてその%sを押せなかった.", xname(otmp));
                    ...
                    : IS_TREE(lev->typ)
                        ...
                        : IS_OBSTRUCTED(lev->typ)
                            ...
                            : (lev->typ == IRONBARS)
                                ? "鉄格子"
                                : "扉",
                svc.context.digging.chew ? "噛み続けた" : "噛み始めた");
        svc.context.digging.chew = TRUE;
        watch_dig((struct monst *) 0, x, y, FALSE);
        return 1;
    }
    ...
    int mode)
{
        ...
        if (Passes_walls && may_passwall(x, y)) {
            ...
            if (mode == DO_MOVE) {
                if (is_db_wall(x, y)) {
                    ...
                    pline_The("Sokobanの壁はあなたの能力に抵抗した.");
                    ...
                    if (sym == S_stone)
                        ...
                        Sprintf(buf, "impossible [background glyph=%d]",
                                glyph);
                    pline_dir(xytodir(dx, dy), "%sだ.", buf);
                }
            }
            return FALSE;
        }
 ...
 testdiag:
    ...
    if (dx && dy && bad_rock(gy.youmonst.data, ux, y)
        ...
        if (mode == DO_MOVE)
            pline("%sが道を塞いでいた.", YMonnam(m_at(ux, y)));
        return FALSE;
    }
    ...
    if (sobj_at(BOULDER, x, y) && (Sokoban || !Passes_walls)) {
        if (mode != TEST_TRAV && svc.context.run >= 2
            ...
            if (mode == DO_MOVE && flags.mention_walls)
                pline_dir(xytodir(dx,dy), "巨大な岩が道を塞いでいた.");
            return FALSE;
        }
        if (mode == DO_MOVE) {
    ...
    struct trap *desttrap) /* nonnull if another trap at <x,y> */
{
    ...
    case TT_WEB:
        ...
        if (--u.utrap) {
            ...
            if (u.usteed)
                pline("%sは巣から抜け出した.", upstart(steedname));
            else
                You("巣から抜け出した.");
        }
        break;
    ...
    case TT_BURIEDBALL:
        ...
        if (--u.utrap) {
            if (flags.verbose) {
                ...
                if (u.usteed) {
                    if (anchored)
                        Norep("あなたと%sは%s.", steedname, predicament);
                    else
                        Norep("%sは%s%s.", upstart(steedname), culprit,
                              predicament);
                } else
 ...
 wriggle_free:
            if (u.usteed)
                pline("%sはついに%s.", upstart(steedname),
                      !anchored ? "もがいて抜け出した"
                                : "鉄球を引きちぎって自由になった");
            else
                You("ついに%s.",
...
domove_fight_web(coordxy x, coordxy y)
{
    ...
    if (svc.context.forcefight && trap && trap->ttyp == WEB && trap->tseen) {
        ...
        if (uwep && (u_wield_art(ART_STING)
            ...
            /* guaranteed success */
            pline("%sは巣を%s!", bare_artifactname(uwep),
                u_wield_art(ART_STING) ? "切り裂いた" : "焼き払った");
    ...
    coordxy x, coordxy y)
{
    ...
    if (mtmp->mtrapped && is_pit(trap->ttyp)
        ...
        You("止まった.  %sは場所を交換したくなかった.", YMonnam(mtmp));
        didnt_move = TRUE;
    } else {
        mtmp->mtrapped = 0;
        remove_monster(x, y);
        ...
        case Trap_Killed_Mon:
             ...
             */
            if (rn2(4)) {
                You_feel("こんな形でペットを失ったことに罪悪感を覚えた.");
                u.ugangr++;
                adjalign(-15);
            }
            break;
...
air_turbulence(void)
{
    if (Is_airlevel(&u.uz) && rn2(4) && !Levitation && !Flying) {
        ...
        case 0:
            ...
            break;
        case 1:
            You_cant("体の動きをうまく制御できなかった.");
            break;
        case 2:
            pline("希薄な空気の中を歩くのは難しかった.");
            exercise(A_DEX, TRUE);
            break;
        }
        return TRUE;
    ...
    boolean newspot) /* true if called by spoteffects */
{
    ...
    if (u.uinwater) {
        ...
        if (!is_pool(u.ux, u.uy)) {
            ...
            You("%sからコルクのように飛び出た!", hliquid("water"));
        } else if (Flying) {
            You("%sから飛び出た.", hliquid("water"));
        } else if (Wwalking) {
            You("ゆっくりと水面上に浮かび上がった.");
        } else {
            still_inwater = TRUE;
        }
        if (!still_inwater) {
...
spoteffects(boolean pick)
{
    ...
    if (Warning && is_ice(u.ux, u.uy)) {
        static const char *const icewarnings[] = {
            "氷がとても柔らかく、ぬかるんでいるようだ.",
            "足元の氷が動くのを感じた!",
            "氷が、割れるぞ!", /* The Dead Zone */
        };
    ...
    if ((mtmp = m_at(u.ux, u.uy)) != 0 && !u.uswallow) {
        ...
        case S_PIERCER:
            pline("%sが%sから突然落ちてきた!", Amonnam(mtmp),
                  ceiling(u.ux, u.uy));
        ...
        default: /* monster surprises you. */
            if (mtmp->mtame)
                pline("%sが%sからあなたのそばへ跳び出してきた.", Amonnam(mtmp),
                      ceiling(u.ux, u.uy));
            else if (mtmp->mpeaceful) {
                You("%sを驚かせた!",
...
pickup_checks(void)
{
    ...
    if (is_pool(u.ux, u.uy)) {
        if (Wwalking || is_floater(gy.youmonst.data)
            ...
            You_cant("底すら見えず、まして%sは拾えなかった.", something);
            return 0;
        }
    }
    if (is_lava(u.ux, u.uy)) {
        if (Wwalking || is_floater(gy.youmonst.data)
            || is_clinger(gy.youmonst.data) || (Flying && !Breathless)) {
            You_cant("底に届かず物を拾えなかった.");
            return 0;
        } else if (!likes_lava(gy.youmonst.data)) {
            You("物を拾おうとすると黒焦げに焼いてしまった.");
            return 0;
        }
    }
    if (!OBJ_AT(u.ux, u.uy)) {
...
maybe_wail(void)
{
    ...
    if (Role_if(PM_WIZARD) || Race_if(PM_ELF) || Role_if(PM_VALKYRIE)) {
        ...
        if (u.uhp == 1) {
            ...
            pline((powercnt >= 4) ? "%s、全ての力が失われようとしていた..."
                                  : "%s、生命力が尽きかけていた.",
                  who);
        }
    } else {
...
showdamage(int dmg)
{
    ...
    pline("[HP %i, 残り %i]", -dmg, Upolyd ? u.mh : u.uhp);
}
