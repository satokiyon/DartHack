    ...
    while (1) {
        ...
        case 0:
            ...
            if (item) {
                ...
                Sprintf(buf, "%sの%s", jp_material_name_for_display(mat_idx),
    ...
    int ef_flags)
{
    static NEARDATA const char
        *const action[] = { "焦げた", "錆びた", "腐った", "腐食した", "ひび割れた" },
        *const msg[] = { "燃え尽きた", "錆びついた", "腐り果てた", "腐食しきった", "ひび割れた" },
    ...
    unsigned ftflags)
{
    ...
    if (td) {
        ...
        pline_The("%sがあなたの下で開きました!", surface(u.ux, u.uy));
    ...
    if (Sokoban && Can_fall_thru(&u.uz)) {
        ...
        dont_fall = "大きすぎて通れなかった.";
    } else if (!next_to_u()) {
        dont_fall = "ペットに引き戻された!";
    }
    if (dont_fall) {
        You1(dont_fall);
    ...
    if (Is_stronghold(&u.uz)) {
        ...
        if (dist > 1)
            You("%s%s%s縦穴へ落ちた!",
                controlled_flight ? "飛んで" : "落下して",
                dist > 3 ? "とても" : "",
                dist > 2 ? "深い" : "");
    }
    if (!td)
        Sprintf(msgbuf, "\u4e0a\u306e%s\u306e\u7a93\u304c\u3075\u3055\u304c\u308b.",
                ceiling(u.ux, u.uy));
...
{
    static const char
        historic_statue_is_gone[] = "歴史的な彫像が失われた";
    ...
    comes_to_life = !canspotmon(mon) ? "\u6d88\u3048\u305f"
                    : golem_xform ? "\u8089\u306b\u5909\u308f\u3063\u305f"
    ...
    if (u_at(x, y) || cause == ANIMATE_SPELL) {
        ...
        Sprintf(statuename, "%s%s", shk_your(tmpbuf, statue),
                (cause == ANIMATE_SPELL
                 ...
                 && (mon != shkp || carried(statue)))
                   ? xname(statue)
                   : "彫像");
        pline("%sが%s!", upstart(statuename), comes_to_life);
    } else if (Hallucination) { /* They don't know it's a statue */
        pline_The("%sが突然生き生きして見えた.", rndmonnam((char *) 0));
        ...
        pline("砕け散る代わりに、%sが突然%s!", statuename,
              comes_to_life);
    ...
    if (!svc.context.mon_moving) {
        ...
        if (historic) {
            You_feel("%sことに罪悪感を覚えた.", historic_statue_is_gone);
            adjalign(-1);
        }
    } else {
        if (historic && cansee(x, y))
            You_feel("%sことを後悔した.", historic_statue_is_gone);
    ...
    if (amorphous(mptr) || is_whirly(mptr) || flaming(mptr)
        ...
        if (flaming(mptr) || acidic(mptr)) {
            if (domsg) {
                if (isyou)
                    You("%sクモの巣を%sた!",
                        a_your[trap->madeby_u], (flaming(mptr)) ? "焼い" : "溶かし");
                else
                    pline_mon(mtmp,
                          "%sは%sクモの巣を%sた!", l_monnam(mtmp),
                          a_your[trap->madeby_u], (flaming(mptr)) ? "焼い" : "溶かし");
            }
            deltrap(trap);
            newsym(x, y);
        ...
        if (domsg) {
            if (isyou) {
                You("%sクモの巣をすり抜けた.", a_your[trap->madeby_u]);
            } else {
                pline_mon(mtmp,
                      "%sは%sクモの巣をすり抜けた.", l_monnam(mtmp),
    ...
    unsigned int trflags UNUSED)
{
 ...
 uglovecheck:
        ...
        case 2:
            ...
            goto uglovecheck;
        default:
            pline("%sあなたを打った!", A_gush_of_water_hits);
 ...
 mglovecheck:
        ...
        default:
            if (in_sight)
                pline("%s%sを打った!", A_gush_of_water_hits, mon_nam(mtmp));
    ...
    unsigned int trflags)
{
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if (!steedintrap(trap, (struct obj *) 0)) {
            if (relevant_spikes) {
                ...
                losehp(Maybe_Half_Phys(rnd(conj_pit ? 4 : adj_pit ? 6 : 10)),
                       ...
                       plunged
                                             ? "故意に鉄の棘だらけの落とし穴へ飛び込んだ"
                       : (conj_pit || deliberate)
                                                 ? "鉄の棘だらけの落とし穴へ足を踏み入れた"
                         : adj_pit
                                                     ? "鉄の棘だらけの落とし穴へつまずいて落ちた"
                                                     : "鉄の棘だらけの落とし穴へ落ちた",
                       NO_KILLER_PREFIX);
                if (!rn2(6))
        ...
        if (!grounded(mptr) || (mtmp->wormno && count_wsegs(mtmp) > 5)) {
            if (forcetrap && !Sokoban) {
                ...
                if (in_sight) {
                    seetrap(trap);
                    pline_mon(mtmp,
                             "%sは落とし穴に落ちなかった.", l_monnam(mtmp));
                }
                return Trap_Effect_Finished;
            }
            if (!inescapable)
                return Trap_Effect_Finished; /* avoids trap */
            fallverb = "引きずり込まれた"; /* sokoban pit */
        }
        ...
        if (in_sight) {
            ...
            pline_mon(mtmp,
                     "%sは%s%s落とし穴へ落ちた!", l_monnam(mtmp), fallverb,
                     a_your[trap->madeby_u]);
    ...
    unsigned int trflags)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (webmaker(gy.youmonst.data)) {
            if (webmsgok)
                pline(trap->madeby_u ? "自分のクモの巣の上を歩いた."
                      : "ここにはクモの巣がある.");
            return Trap_Effect_Finished;
        }
        if (webmsgok) {
        ...
        /* Time stuck in the web depends on your/steed's strength. */
        {
            ...
            if (str <= 3)
                ...
                tim = 0;
                if (webmsgok)
                    You("%sクモの巣を引き裂いた!", a_your[trap->madeby_u]);
                deltrap(trap);
    ...
    unsigned int trflags)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (viasitting)
            Strcpy(verbbuf, "作動させた"); /* follows "You sit down." */
        else if (u.usteed)
            Sprintf(verbbuf, "%sを連れて",
                    x_monnam(u.usteed, steed_article, (char *) 0,
                             SUPPRESS_SADDLE, FALSE));
        else
...
#define MINE_TRIGGER_WT (WT_ELF / 2U)
        ...
        if (m_in_air(mtmp)) {
            ...
            pline_mon(mtmp,
                  "%s%sは%s地雷を起爆した!",
                  !Deaf ? "ドカーン!!!  " : "", Monnam(mtmp),
                  a_your[trap->madeby_u]);
        }
        if (!in_sight && !Deaf)
            pline("ドカーン!  遠くで爆発音が%s!",
                  "聞こえた");  /* Deaf-aware */
        blow_up_landmine(trap);
    ...
    unsigned int trflags UNUSED)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (!m_in_air(mtmp)) {
            ...
            newsym(mtmp->mx, mtmp->my);
            if (in_sight)
                pline_mon(mtmp, "%s%sが%sを起動した.",
                      !Deaf ? "カチッ! " : "", Monnam(mtmp),
                      trap->tseen ? "転がる巨大な岩の罠" : something);
            if (launch_obj(BOULDER, trap->launch.x, trap->launch.y,
                           trap->launch2.x, trap->launch2.y, style)) {
                if (in_sight)
                    trap->tseen = TRUE;
    ...
    case SLP_GAS_TRAP:
        if (!resists_sleep(steed) && !breathless(steed->data)
            ...
            if (sleep_monst(steed, rnd(25), -1))
                ...
                pline("%sは突然眠り込んだ!", l_monnam(steed));
        }
        steedhit = TRUE;
        break;
    case LANDMINE:
    ...
    while (dist-- > 0 && !used_up) {
        ...
        if (style == ROLL) {
            ...
            if (otyp == BOULDER && (otmp2 = sobj_at(BOULDER, x, y)) != 0) {
                const char *bmsg = "（ぶつかった岩で別の岩が動き出した）";
                ...
                You_hear("激しい衝突音がした%s!", cansee(x, y) ? bmsg : "");
    ...
    if (!trap) {
        ...
        if (!rn2(40) || (is_pit(trap->ttyp) && m_easy_escape_pit(mtmp))) {
            if (sobj_at(BOULDER, mtmp->mx, mtmp->my)
                ...
                if (canseemon(mtmp)) {
                    set_msg_xy(mtmp->mx, mtmp->my);
                    if (is_pit(trap->ttyp))
                        pline("%sは%s落とし穴から這い出した.", l_monnam(mtmp),
        ...
        if (!DEADMONSTER(mtmp) && mtmp->mtrapped) {
            ...
            if (!alreadyspotted && canseemon(mtmp))
                pline_mon(mtmp, "%sが姿を現した.", Amonnam(mtmp));
        }
    }
    return trap_result;
}
...
minstapetrify(struct monst *mon, boolean byplayer)
{
    ...
    if (cansee(mon->mx, mon->my))
        pline_mon(mon, "%sは石になった.", l_monnam(mon));
    if (byplayer) {
        gs.stoned = TRUE;
...
float_up(void)
{
    ...
    if (u.utrap) {
        ...
        if (is_animal(u.ustuck->data))
            You("%sからふわりと離れた.", surface(u.ux, u.uy));
        else
            You("らせんを描いて%sの中へ浮かび上がった.", l_monnam(u.ustuck));
    } else if (Hallucination) {
        pline("上へ、上へ、さらに上へ! 空中を歩いている! ");
    } else if (Is_airlevel(&u.uz)) {
        You("体の動きを制御できるようになった.");
    } else {
        You("空中に浮かび始めた!");
    }
    if (u.usteed && !is_floater(u.usteed->data)
        ...
        if (Lev_at_will) {
            pline("%sが魔法でふわりと浮かび上がった!", l_monnam(u.usteed));
        } else {
            You("%sの上に乗っていられなかった.", l_monnam(u.usteed));
    ...
    long emask) /* might cancel timeout */
{
    ...
    if (BLevitation) {
        ...
        if (trapped && u.utrap) /* u.utrap => paranoia */
            You("%sから浮かび上がろうとするのをやめた.",
                (u.utraptype == TT_BEARTRAP) ? "罠のあご"
                  : (u.utraptype == TT_WEB) ? "クモの巣"
    ...
    if (u.uswallow) {
        You("降下したが、まだ%s状態だった.",
            digests(u.ustuck->data) ? "飲み込まれた" : "包み込まれた");
        encumber_msg();
        return 1;
    }
    ...
    if (!Flying) {
        if (!u.uswallow && u.ustuck) {
            if (sticks(gy.youmonst.data))
                ...
                pline("驚いた%sは、もうあなたをつかんでいられなかった!",
                      l_monnam(u.ustuck));
        ...
        /* u.uinwater msgs already in spoteffects()/drown() */
            ...
            if (!(emask & W_SADDLE)) {
                if (Sokoban && trap) {
                    ...
                    You("%sへゆっくり降りた.", surface(u.ux, u.uy));
                }
            }
        }
    }
...
climb_pit(void)
{
    ...
    if (Passes_walls) {
        ...
        if (u.usteed)
            Norep("%sはまだ落とし穴の中にいる.", YMonnam(u.usteed));
        else
...
domagictrap(void)
{
    ...
    if (fate < 10) {
        ...
        case 16:
            ...
            break;
        case 17:
            You(Hallucination ? "ハンバーガーの匂いがした." : "焦げた肉の匂いがした.");
            break;
        case 18:
            You_feel("疲れを覚えた.");
            break;
    ...
    if (objects[otyp].oc_material < DRAGON_HIDE
        ...
        if (cansee(x, y)) {
            ...
            if (obj == gt.thrownobj || obj == gk.kickedobj)
                pline("%s!", is_plural(obj) ? "それらは燃え尽きた" : "それは燃え尽きた");
            else
                You_see("%sは溶岩に触れて燃え尽きた!", doname(obj));
        }
...
{
    const char *surf = surface(u.ux, u.uy), *jsurf = surf, *you_are_back;
    char icebuf[QBUFSZ];

    if (is_ice(u.ux, u.uy)) {
        (void) ice_descr(u.ux, u.uy, icebuf);
        jsurf = "氷の上";
    } else if (!strcmpi(surf, "floor") || !strcmpi(surf, "ground")) {
        jsurf = "地面";
    } else if (!strcmpi(surf, "bridge") || !strcmpi(surf, "altar")
               || !strcmpi(surf, "headstone")) {
        jsurf = !strcmpi(surf, "bridge") ? "橋の上"
               : !strcmpi(surf, "altar") ? "祭壇の上"
               : "墓石の上";
    } else if (!strcmpi(surf, "stairs") || !strcmpi(surf, "lava")
               || !strcmpi(surf, "bottom")) {
        jsurf = !strcmpi(surf, "stairs") ? "階段の上"
               : !strcmpi(surf, "lava") ? "溶岩の上"
               : "底";
    } else {
        jsurf = !strcmpi(surf, "cloud") ? "雲の中"
               : !strcmpi(surf, "air") ? "空中"
               : !strcmpi(surf, "air bubble") ? "空気の泡の中"
               : !strcmpi(surf, "wall") ? "壁の中"
               : !strcmpi(surf, "fountain") ? "噴水の中"
               : !strcmpi(surf, "doorway") ? "戸口"
               : surf;
    }
    if (rescued) {
        you_are_back = "気がつくと";
    } else {
        you_are_back = flags.verbose ? "あなたは戻っていた" : "戻っていた";
    }
    pline("%s%sにいた.", you_are_back, jsurf);
...
{
    static const char find_yourself[] = "気がつくと";
...
drown(void)
{
    ...
    if ((i = number_leashed()) > 0) {
        pline_The(i > 1 ? "リードがいくつも外れた." : "リードが外れた.");
        unleash_all();
    }
    ...
    if ((Teleportation || can_teleport(gy.youmonst.data)) && !Unaware
        ...
        You("テレポート呪文を試みた."); /* utcsri!carroll */
        if (!noteleport_level(&gy.youmonst)) {
            (void) dotele(FALSE);
            if (!is_pool(u.ux, u.uy))
                return TRUE;
        } else
            pline_The("試みたテレポート呪文は失敗した.");
    }
    if (u.usteed) {
        dismount_steed(DISMOUNT_GENERIC);
        if (!is_pool(u.ux, u.uy))
...
could_untrap(boolean verbosely, boolean check_floor)
{
    ...
    if (near_capacity() >= HVY_ENCUMBER) {
        ...
        Sprintf(buf, "%sには手が届かなかった.", surface(u.ux, u.uy));
    }
    if (buf[0]) {
        if (verbosely)
            pline("%s", buf);
...
move_into_trap(struct trap *ttmp)
{
    ...
    if (test_move(u.ux, u.uy, sgn(x - u.ux), sgn(y - u.uy), TEST_MOVE)
        ...
        pline("幸い、%sはしなかった.",
              into_vs_onto(ttmp->ttyp) ? "中へ移動" : "上へ移動");
    }
}
            ...
            if (mtmp) { /* must be a trap that holds monsters */
                if (ttype == BEAR_TRAP) {
                    ...
                    if (!webmaker(gy.youmonst.data)
                        ...
                        pline_The("クモの巣があなたに張り付き、あなたも捕まった!");
                    ...
                    if (mtmp->mtrapped)
                        pline("%sはまだ絡まれたままだ.", l_monnam(mtmp));
                }
            } else if (under_u) {
...
disarm_holdingtrap(struct trap *ttmp)
{
    ...
    if ((mtmp = m_at(ttmp->tx, ttmp->ty)) != 0) {
        ...
        if (wep && wep->oartifact
            ...
            pline("%sが%s%sクモの巣を%s!", bare_artifactname(uwep),
                  which, "", u_wield_art(ART_STING) ? "切り裂いた" : "焼き払った");
        else if (wep)
            You("%sクモの巣を切り裂いた.", which);
        else
            You("%sクモの巣の除去に成功した.", which);
        deltrap(ttmp);
    }
...
disarm_landmine(struct trap *ttmp) /* Helge Hafting */
{
    ...
    if (fails < 2)
        return fails;
    You("%s地雷を解除した.", the_your[ttmp->madeby_u]);
...
disarm_shooting_trap(struct trap *ttmp, int otyp)
{
    ...
    if (fails < 2)
        return fails;
    You("%s罠を解除した.", the_your[ttmp->madeby_u]);
    ...
    boolean stuff) /* False: monster w/o minvent; True: w/ minvent */
{
    if (calc_capacity(xtra_wt) >= HVY_ENCUMBER) {
        ...
        if (!ttmp->madeby_u && !mtmp->mpeaceful && mtmp->mcanmove
            ...
            mtmp->mpeaceful = 1;
            set_malign(mtmp); /* reset alignment */
            pline("%sはあなたの試みをありがたいと思ったようだ.", l_monnam(mtmp));
        }
        return 0;
    }
    return 1;
    ...
    struct trap *ttmp)
{
     ...
     */
    if (!mtmp->mtrapped) {
        pline("%sは罠にかかっていない.", l_monnam(mtmp));
        return 0;
    }
    ...
    if (uprob) {
        You("%sをつかもうとしたが、うまくつかめなかった.", l_monnam(mtmp));
        if (mtmp->msleeping) {
            mtmp->msleeping = 0;
            pline("%sは目を覚ました.", l_monnam(mtmp));
        }
        return 1;
    }
    ...
    You("%sを落とし穴から引き上げた.", l_monnam(mtmp));
    mtmp->mtrapped = 0;
    reward_untrap(ttmp, mtmp);
    fill_pit(mtmp->mx, mtmp->my);
    return 1;
    ...
    if (autounlock_door) {
        ...
        *the_trap = '\0';
        ...
        if (ttmp && boxcnt)
            Strcat(the_trap, "と");
        if (boxcnt)
            Strcat(the_trap, (boxcnt == 1) ? "容器" : "複数の容器");
        useplural = ((ttmp && boxcnt > 0) || boxcnt > 1);
        /* note: boxcnt and useplural will always be 0 for !here case */
        if (ttmp || boxcnt)
            There("%s%s%sがあるが、%sには手が届かない%s.",
                here ? "ここに" : "そこに", the_trap, useplural ? "" : "",
                useplural ? "それら" : "それ",
                u.usteed ? "（騎乗中）" : "");
        trap_skipped = (ttmp != 0);
        ...
        if (ttmp) {
            ...
            if (deal_with_floor_trap) {
                if (u.utrap) {
                    You("罠にかかったままでは%sを扱えない%s!", the_trap,
                        u_at(x, y) ? "（中にいる）" : "");
                    return 1;
                }
                if ((mtmp = m_at(x, y)) != 0
    ...
    if (ishero) {
        ...
        *noticed = TRUE;
        if (!u.usteed)
            ...
            Sprintf(buf, "あなたと%sは", y_monnam(u.usteed));
        else
    ...
    boolean disarm)
{
    ...
    if (Luck > -13 && rn2(13 + Luck) > 7) { /* saved by luck */
        ...
        case 21: {
            ...
            if (costly && loss) {
                if (insider) {
                    You("壊した物の弁償として%ld%sを支払う必要がある.", loss,
                        currency(loss));
                } else {
                    You("%ld%s相当の損害を与えた!", loss,
                        currency(loss));
    ...
    boolean nocorpse)  /* True: a trap is completely burning up the target */
{
    ...
    if (!strike) {
        if (obj && cansee(mon->mx, mon->my))
            pline_mon(mon, "%sは%sにかすめられた!",
                      l_monnam(mon), doname(obj));
    } else {
        int dam = 1;
...
lava_effects(void)
{
    ...
    if (!Fire_resistance) {
        if (Wwalking) {
            ...
            You("%sへ落ちた!", waterbody_name(u.ux, u.uy));
        ...
        if (usurvive && burncount > burnmesgcount)
                        pline("所持品のうち%d個が焼失した.", burncount - burnmesgcount);
        ...
        for (burncount = 0; burncount < 2; ++burncount) {
            ...
            urgent_pline("あなたは%s...", boil_away ? "煮え崩れ"
                                                : "こんがり焼け");
            done(BURNING);
            if (safe_teleds(TELEDS_ALLOW_DRAG | TELEDS_TELEPORT))
                break; /* successful life-save */
        ...
        set_utrap((unsigned) (rn1(4, 4) + ((boil_away ? 2
                                                      : rn1(4, 12)) << 8)),
                  TT_LAVA);
        You("%sに沈んだ%s!", waterbody_name(u.ux, u.uy),
            !boil_away ? "が、熱さはわずかだった"
                       : ".このままでは焼き尽くされる");
        if (Fire_resistance)
            monstseesu(M_SEEN_FIRE);
        else
...
sink_into_lava(void)
{
    static const char sink_deeper[] = "あなたはさらに溶岩へ沈んでいく.";
    ...
    boolean override) /* if True, ignore Hallucination */
{
    ...
    if (Hallucination && !override) {
        ...
        if (nameidx == total_names) {
            ...
            copynchars(roletrap,
                       rn2(3) ? jp_role_name_for_display(flags.initrole, fem ? 1 : 0)
