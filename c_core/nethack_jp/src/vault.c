    ...
    if (!guard) {
        ...
        if (u.uswallow) {
            ...
            if (!Deaf) {
                ...
                verbalize("ここで何が起きている?");
            }
            if (!spotted)
                pline_The("もう一つの気配は消えた.");
            mongone(guard);
            return;
        }
        ...
        if (Strangled || is_silent(gy.youmonst.data) || gm.multi < 0) {
            ...
            if (Deaf) {
                pline("%sはいらだった様子で向きを変えて去ろうとした.",
                      noit_Monnam(guard));
            } else {
                SetVoice(guard, 0, 80, 0);
                verbalize("話せるようになったらまた来る!");
            }
            mongone(guard);
            return;
        }
        ...
        if (!strcmpi(buf, "Croesus") || !strcmpi(buf, "Kroisos")
            || !strcmpi(buf, "クロイソス") /* 日本語入力対応 */
            || !strcmpi(buf, "Creosote") || !strcmpi(buf, "クレオソート")) { /* Discworld（Croesusのパロディキャラ）*/
            if (!svm.mvitals[PM_CROESUS].died) {
                if (Deaf) {
                    if (!Blind)
                        pline("%sは手を振って去った.", noit_Monnam(guard));
                } else {
                    SetVoice(guard, 0, 80, 0);
                    verbalize(
                         "ああ、もちろんです. お邪魔して申し訳ない.");
                }
                mongone(guard);
            } else {
                ...
                if (Deaf) {
                   if (!Blind)
                        pline("%sは何かを口パクで告げ、ひどく怒っているようだった!",
                              noit_Monnam(guard));
                } else {
                   SetVoice(guard, 0, 80, 0);
                   verbalize(
                           "死から戻ってきたのか? ならば片を付けてやる!");
                }
        ...
        if (Deaf) {
            pline("%sはあなたに%s見覚えがなかった.", noit_Monnam(guard),
                  (Blind) ? "" : "どうやら");
        } else {
            SetVoice(guard, 0, 80, 0);
            verbalize("お前を知らない.");
        ...
        if (!umoney && !hidden_gold(TRUE)) {
            if (Deaf) {
                pline("%sは足を踏み鳴らした%s.", noit_Monnam(guard),
                      (Blind) ? "" : "り、手招きした");
            } else {
                SetVoice(guard, 0, 80, 0);
                verbalize("ついて来い.");
            ...
            if (Deaf) {
                ...
                SetVoice(guard, 0, 80, 0);
                verbalize(
                    "おそらく、その金はすべてこの宝物庫から盗んだものだ.");
                SetVoice(guard, 0, 80, 0);
                verbalize("その金を置いて、ついて来い.");
            }
...
gd_pick_corridor_gold(struct monst *grd, int goldx, int goldy)
{
    ...
    if (see_it) { /* cansee(goldx, goldy) */
          pline("%s%s金を拾った%s.", Some_Monnam(grd),
...
gd_move(struct monst *grd)
{
    ...
    if (egrd->witness) {
        if (!Deaf) {
            SetVoice(grd, 0, 80, 0);
            verbalize("よくもその金を%sしたな、この悪党め!",
                      (egrd->witness & GD_EATGOLD) ? "食い物に" : "台無しに");
        }
        egrd->witness = 0;
        grd->mpeaceful = 0;
        return -1;
    ...
    if (egrd->fcend == 1) {
        if (u_in_vault && (u_carry_gold || um_dist(grd->mx, grd->my, 1))) {
            if (egrd->warncnt == 3 && !Deaf) {
                ...
                Sprintf(buf, "%sついて来い!",
                        u_carry_gold ? (!umoney ? "隠し持った金を置いて、"
                                                : "その金を置いて、")
                                     : "");
                SetVoice(grd, 0, 80, 0);
                if (egrd->dropgoldcnt || !u_carry_gold)
                    verbalize("繰り返すが、%s", buf);
                else
    ...
    if (egrd->fcend > 1) {
        if (egrd->fcend > 2 && in_fcorridor(grd, grd->mx, grd->my)
            ...
            pline("%sは混乱して消えた.", noit_Monnam(grd));
