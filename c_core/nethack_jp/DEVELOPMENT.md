...
### 英語モードにおける UTF-8 入出力の考慮
- **原則**: 英語モード（`nethack_en`）であっても、プレイヤー名・ペット名・果物名・階層注釈・各種オプション設定やログ出力など、ユーザーの入出力には UTF-8（日本語文字、マルチバイト記号、絵文字など）が含まれることを常に考慮して設計・実装してください。
- **対策**: ASCII 前提の単バイト切り詰め（`strncpy` や `%.*s` による文字途中切断）は文字化けやファイル名・バッファ破損の原因となるため、英語コアにおいても必ず `utf8_truncate()` 等を用いて安全な文字境界での処理を行ってください。また、セーブデータの日英間完全互換を維持するため、文字列バッファサイズ（`PL_FSIZ` など）や構造体定義も日本語コアと完全に同期させてください。
