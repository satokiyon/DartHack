...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "===> 本家 NetHack との重要ファイル（extern.h）の宣言整合性を検証中..." -ForegroundColor Cyan
    try {
        $upstreamExtern = git show "nethack-en/${Branch}:include/extern.h"
        $localExtern = Get-Content "c_core/nethack_en/include/extern.h" -Raw
        ...
        if ($missingCount -gt 0) {
            Write-Host "[WARNING] 本家 NetHack に存在するが c_core/nethack_en/include/extern.h に含まれていない extern 宣言が $missingCount 件検出されました！" -ForegroundColor Yellow
