...
2. **主人公・ペット・果物等の名前バッファ定数と有効上限バイト数の二重コア仕様**:
   - CコアおよびFlutter設定画面における各項目の有効上限バイト数（末尾 `\0` を除く）は以下を厳守してください。
     ...
     - **果物の名前 (`svp.pl_fruit`)**:
       - 日本語コア (`nethack_jp`): `PL_FSIZ` = 64（有効上限 **63 バイト**）
       - 英語コア (`nethack_en`): `PL_FSIZ` = 32（有効上限 **31 バイト**）
       - Flutter 設定画面では現在の言語モード（`Localizations.localeOf(context).languageCode == 'ja'`）を判定し、日本語時は 64、英語時は 32 を動的に切り替えて渡す設計を徹底してください。
