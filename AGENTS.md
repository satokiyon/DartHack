...
14. **`You_hear()` 等の知覚マクロにおける体言止め・文途切れ防止と完全文化**:
    - `You_hear(str)` は自動的に「あなたには」を前置して出力します。英語の `You hear <noun phrase>.` （例: `You hear Ebenezer Scrooge!`, `You hear a crash.`）をそのまま名詞句で直訳すると、「あなたにはエベネーザ・スクルージ!」「あなたには激しい破壊音.」のように文末が体言止めで途切れてしまいます。
    - 知覚マクロ（`You_hear`, `You_see`, `You_feel` 等）に渡す文字列には、必ず文脈に応じた適切な述語（例: 「誰かが「〜!」と叫ぶ声が聞こえる!」「〜音が聞こえた.」「〜のを見た.」）を補い、プレフィックスと接続して完全な日本語文として成立するように設計してください。

15. **モンスター・ペットの行動・威嚇音における主格（能動動詞）表現と主客転倒防止**:
    - **ペット・モンスター主格の動詞表現**: モンスターやペットが攻撃された際の威嚇・鳴き声（`growl()` 等）を表示する際は、プレイヤー主格の聴覚描写（「〜のうなり声が聞こえた!」等）ではなく、対象の生物が主格となる能動動詞表現（例: 「小犬はうなった!」「大型犬は咆哮した!」「子猫はシューッと威嚇した!」）を用いて行動を直接描写してください。
    - **状態異常時の主客関係の厳守**: プレイヤー自身が難聴（`Deaf`）の状態で話しかけた際（`Any response from %s falls on deaf ears.`）など、助詞の選択ミスにより「返事は相手には届かなかった」のように主客が逆転するのを防ぎ、「相手からの返事は耳に届かなかった / 聞こえなかった」と、誰が誰の行動・返答を知覚できないのかを正確に記述してください。
...
## エラー調査・障害報告におけるファクトベース分析の徹底方針

1. **一次情報（ログ出力）と推測・補足対応の厳格な分離**:
   - ビルドエラーや CI 失敗の原因を調査・報告する際は、**ログに出力された実際のエラー行（一次情報）**を引用して提示し、自身の推論や Web 検索結果による補足情報と明確に区別してください。
   - ログに記録されていない事象を「ログでエラーが発生していた」と事実と異なる報告をしてはなりません。
   - ログには直接現れていないが関連する問題（例: ログで停止したステップ以降で発生が予見されるパッケージ名の非推奨や環境差異等）を予防的・併せて修正した場合は、「ログには直接現れていないが、〜の観点から念のため修正した」と事実関係を正確に説明してください。

## リポジトリ・コミットにおける作業分離とコミット対象の厳格限定方針

1. **`git add .` の禁止とタスク対象ファイルのみの明示的ステージング**:
   - ユーザーが別セッションで編集中の一時変更や未完成のファイルが存在する可能性があるため、`git add .`, `git add -A`, `git commit -a` の使用は厳禁とします。
   - 必ずタスクに関連する本質的な成果物のみを `git add <ファイルパス>` で個別に指定してステージングしてください。
   - ユーザーから特定のファイルを除外する指示があった場合は、コミット直前に `git status` を確認し、対象外ファイルが未ステージング状態のままであることを厳格に担保してください。

## Linux / WSL / CI 環境におけるビルド依存・Lua 取得・ワークフロー記述方針

1. **Ubuntu 24.04 (Noble) におけるパッケージ名の仕様**:
   - UUID 開発パッケージは `libuuid-dev` ではなく **`uuid-dev`** を指定してください。
   - フォント変換ツール `bdftopcf` は独立パッケージとしては存在しないため、**`xfonts-utils`** を指定してください（`bdftopcf` および `mkfontdir` が内包されています）。
   - ncurses 開発パッケージは Ubuntu 24.04 では `libncurses-dev` を指定してください。

2. **NetHack Unix ビルドにおける Lua ソース取得と submodules/CHKSUMS の保持**:
   - NetHack の Unix ビルドシステムは `lib/lua-5.4.8/` をリポジトリに内包しておらず、`make fetch-lua` によるダウンロードが必要です。
   - `make fetch-lua` は `submodules/CHKSUMS` を用いて SHA-256 チェックサムを検証します。`.gitignore` で `submodules/*` を除外する場合でも、必ず **`!submodules/CHKSUMS`** を指定してチェックサム定義ファイルを Git の追跡対象に維持してください。
   - ビルドスクリプト（`sys/unix/build_wsl.sh` 等）では、ネットワークや環境の差異に備え、`make fetch-lua || make fetch-lua NOCHKSUM=1` のようにチェックサムスキップのフォールバックを設けてください。
