...
* **通常オブジェクトの日本語名による願い対応（JNetHack表記揺れ吸収対応）**:
  ...
  - クラス接尾辞（「の巻物」「の指輪」など）が剥ぎ取られた部分名（例: 「鑑定」→ `"magic mapping"` 等）でマッチングするようにマッピングを構成。
  - `src/objnam.c` の `jp_wish_match()` 内で、入力された文字列を `jnh_wish_aliases` を用いて英語キー（正規化文字列）に変換してから、段階的に英語名との比較を行うことで、日本語の通常アイテム名が入力された場合にも正しいオブジェクトが「願い」でヒットするように実装。
